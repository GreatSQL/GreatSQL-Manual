...
GreatSQL 内部维持着一些 Cache 和 Buffer，比如 Query Cache 用来缓存 `SELECT` 的执行结果，如果能够在其中找到对应的查询结果，那么就不必再进行查询解析、优化和执行的整个过程了，可以直接将结果反馈给客户端。
...
在金融级应用场景中，强烈建议采用 GreatSQL MGR 来构建高一致性高可用数据库架构。
