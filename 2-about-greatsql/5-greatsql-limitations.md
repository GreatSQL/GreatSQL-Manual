...
- 要求 Binlog format 务必是 ROW 模式，即 `binlog_format=ROW`。
...
- 要求 `master_info_repository=TABLE` 及 `relay_log_info_repository=TABLE`，不过从 8.0.23 开始，这两个选项已经默认设置 TABLE，因此无需再单独设置。
