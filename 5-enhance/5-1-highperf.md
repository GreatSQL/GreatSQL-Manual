...
3. 支持并行load data，适用于频繁导入大批量数据的应用场景，性能可提升约20+倍。更详细内容参考：[并行load data](5-1-highperf-parallel-load.md)。

4. 支持异步删除InnoDB大表，提高InnoDB引擎运行时性能的稳定性。更详细内容参考：[异步删除InnoDB大表](5-1-highperf-async-purge-big-table.md)。

5. 支持NUMA调度优化，通过将线程分类以及绑定CPU核心等方式，减少跨NUMA的访问，提升GreatSQL运行时性能的稳定性。更详细内容参考：[NUMA调度优化](5-1-highperf-numa-affinity.md)。
