...
在后续输出中的这些消息 ER_YES 的 Code 列中使用 SHOW WARNINGS。
...
如果错误消息涉及错误 150，则表创建失败，因为 未正确形成外键约束。

如果错误消息指示错误-1，则表创建可能失败，因为该表包含与内部 InnoDB 表名称匹配的列名称。
...
如果您确实要替换现有数据库，请先删除数据库，或者如果要保留现有数据库而不使该语句产生错误，请在该语句中添加一个 IF NOT EXISTS 子句 CREATE DATABASE。
...
报错信息格式：删除数据库时出错（无法 rmdir'％s'，错误号：％d-％s）。
...
InnoDB 如果尝试访问 InnoDB INFORMATION_SCHEMA 表 InnoDB 不可用，则 返回。
...
InnoDB 当找不到 InnoDB 数据文件中的表时，报告此错误。
...
ER_FILSORT_ABORT 在 8.0.18 之后被删除。
...
报错信息格式：内存不足；检查 mysqld 或其他进程是否使用了所有可用内存；如果不是，则可能必须使用“ulimit”来允许 mysqld 使用更多的内存，或者可以添加更多的交换空间。
...
可能的原因：查询中没有适当限定的列出现在选择列表或 ON 子句中。
...
报错信息格式：BLOB 列'％s'不能用于已使用表类型的键规范中。

</br>

- 错误号：1074; 符号： ER_TOO_BIG_FIELDLENGTH; SQLSTATE： 42000

报错信息格式：列长度对于列'％s'太大（最大值=％lu）；使用 BLOB 或 TEXT 代替。
...
ER_NORMAL_SHUTDOWN 在 8.0.4 之后被删除。
...
报错信息格式：表'％s'没有类似于 CREATE INDEX 中使用的索引；重新创建表。
...
报错信息格式：您不能对 BLOB 使用固定的行长；请使用“字段终止于”。
...
报错信息格式：记录：％ld 已删除：％ld 跳过：％ld 警告：％ld。
...
报错信息格式：您不能使用 ALTER TABLE 删除所有列；改用 DROP TABLE。
...
报错信息格式：记录：％ld 重复项：％ld 警告：％ld。

</br>

- 错误号：1093; 符号： ER_UPDATE_TABLE_USED; SQLSTATE： HY000

报错信息格式：您无法在 FROM 子句中指定目标表'％s'用于更新。

如果尝试在单个语句中选择并修改同一表，则会发生此错误。

如果选择尝试发生在派生表中，则可以通过设置系统变量的 derived_merge 标志 optimizer_switch 来强制将子查询具体化为临时表，从而有效地使它成为与修改后的表不同的表，从而避免此错误。

请参见第 8.2.2.4 节“通过合并或实现来优化派生表，视图引用和公用表表达式”。
...
报错信息格式：表'％s'被 READ 锁锁定，无法更新。

</br>

- 错误号：1100; 符号： ER_TABLE_NOT_LOCKED; SQLSTATE： HY000

报错信息格式：表'％s'没有被 LOCK TABLES 锁定。

</br>

- 错误号：1101; 符号： ER_BLOB_CANT_HAVE_DEFAULT; SQLSTATE：42000

报错信息格式：BLOB，TEXT，GEOMETRY 或 JSON 列'％s'不能具有默认值。
...
报错信息格式：SELECT 将检查超过 MAX_JOIN_SIZE 行；检查您的 WHERE 并使用 SET SQL_BIG_SELECTS = 1 或 SET MAX_JOIN_SIZE =＃（如果 SELECT 可以）。
...
报错信息格式：表'％s'使用了此 MySQL 版本中不存在的扩展名。
...
ER_WRONG_OUTER_JOIN 在 8.0.0 之后被删除。
...
ER_WRONG_OUTER_JOIN_UNUSED 在 8.0.1 中添加。
...
报错信息格式：表处理程序在给定索引中不支持 NULL。

请更改列'％s'为 NOT NULL 或使用其他处理程序
...
报错信息格式：不允许主机“％s”连接到该 MySQL 服务器。

</br>

- 错误号：1131; 符号： ER_PASSWORD_ANONYMOUS_USER; SQLSTATE：42000

报错信息格式：您将 MySQL 作为匿名用户使用，并且不允许匿名用户更改密码。

</br>

- 错误号：1132; 符号： ER_PASSWORD_NOT_ALLOWED; SQLSTATE： 42000

报错信息格式：您必须具有更新 mysql 数据库中表的权限，才能更改其他用户的密码。
...
报错信息格式：如果没有 GROUP BY 子句，则将 GROUP 列（MIN（），MAX（），COUNT（），...）与 GROUP 列混合使用是非法的。
...
报错信息格式：表'％s'的用户'％s'@'％s'拒绝％s 命令。

</br>

- 错误号：1143; 符号： ER_COLUMNACCESS_DENIED_ERROR; SQLSTATE：42000

报错信息格式：对表'％s'中列'％s'的用户'％s'@'％s'拒绝％s 命令。
...
报错信息格式：此 MySQL 版本不允许使用命令。

</br>

- 错误号：1149; 符号： ER_SYNTAX_ERROR; SQLSTATE： 42000

报错信息格式：您的 SQL 语法有错误；检查与您的 MySQL 服务器版本相对应的手册以使用正确的语法。

</br>

- 错误号：1152; 符号： ER_ABORTING_CONNECTION; SQLSTATE： 08S01

报错信息格式：与数据库的％ld 连接中断：'％s'用户：'％s'（％s）。
...
报错信息格式：无法打开定义不同或非 MyISAM 类型或不存在的基础表。
...
报错信息格式：密钥规范中使用的 BLOB / TEXT 列'％s'没有密钥长度。

</br>

- 错误号：1171; 符号： ER_PRIMARY_CANT_HAVE_NULL; SQLSTATE：42000

报错信息格式：PRIMARY KEY 的所有部分都不能为 NULL；如果键中需要 NULL，请改用 UNIQUE。
...
报错信息格式：您正在使用安全更新模式，并且试图在没有使用 KEY 列的 WHERE 的情况下更新表。
...
报错信息格式：在 FLUSH_LOGS 期间出错％d。

</br>

- 错误号：1184; 符号： ER_NEW_ABORTING_CONNECTION; SQLSTATE：08S01

报错信息格式：与数据库的％u 连接终止：'％s'用户：'％s'主机：'％s'（％s）。
...
报错信息格式：多语句事务需要超过“ max_binlog_cache_size”个字节的存储空间；增加此 mysqld 变量，然后重试。
...
报错信息格式：服务器未配置为从服务器；修复配置文件或使用 CHANGE REPLICATION SOURCE TO。

</br>

- 错误号：1201; 符号： ER_MASTER_INFO; SQLSTATE： HY000

报错信息格式：无法初始化主信息结构；可以在 MySQL 错误日志中找到更多错误消息。
...
报错信息格式：用户％s 已具有多个“ max_user_connections”活动连接。

</br>

- 错误号：1204; 符号： ER_SET_CONSTANTS_ONLY; SQLSTATE： HY000

报错信息格式：您只能将常量表达式与 SET 一起使用。
...
InnoDB 锁定等待超时到期时报告此错误。

等待时间过长的语句已 回滚（不是整个 事务）。

innodb_lock_wait_timeout 如果 SQL 语句应等待更长的时间来等待其他事务完成，则可以增加配置选项的值，或者如果太多长时间运行的事务导致锁定问题并减少 繁忙系统上的并发性，则可以减小配置选项 的值。
...
InnoDB 当锁的总数超过用于管理锁的内存量时，将报告此错误。

为避免此错误，请增加的值 innodb_buffer_pool_size。

在单个应用程序内，一种解决方法可能是将大型操作分解为较小的部分。

例如，如果错误发生在一个大的位置 INSERT，则执行几个较小的 INSERT 操作。
...
报错信息格式：在 READ UNCOMMITTED 事务期间无法获取更新锁。
...
InnoDB 当事务遇到 死锁并自动 回滚时报告此错误， 以便您的应用程序可以采取纠正措施。
...
如果您经常遇到死锁，请执行锁定操作的顺序（LOCK TABLES，SELECT ... FOR UPDATE 等等）在遇到问题的不同事务或应用程序之间保持一致。

有关详细信息，请参见 第 15.7.5 节“ InnoDB 中的死锁”。
...
InnoDB 当您尝试添加行但没有父行，并且外键约束失败时，将报告此错误。
...
InnoDB 当您尝试删除具有子项的父行且外键约束失败时，将报告此错误。
...
报错信息格式：访问被拒绝；您需要（至少一个）％s 权限才能执行此操作。

</br>

- 错误号：1228; 符号： ER_LOCAL_VARIABLE; SQLSTATE： HY000

报错信息格式：变量'％s'是 SESSION 变量，不能与 SET GLOBAL 一起使用。

</br>

- 错误号：1229; 符号： ER_GLOBAL_VARIABLE; SQLSTATE： HY000

报错信息格式：变量“％s”是 GLOBAL 变量，应使用 SET GLOBAL 进行设置。
...
报错信息格式：变量'％s'不是变量组件（不能用作 XXXX.variable_name）。
...
报错信息格式：CHANGE REPLICATION SOURCE 中的 SSL 参数将被忽略，因为该 MySQL 从站是在没有 SSL 支持的情况下编译的；如果启动具有 SSL 的 MySQL 从服务器，则可以在以后使用它们。

</br>

- 错误号：1275; 符号： ER_SERVER_IS_IN_SECURE_AUTH_MODE; SQLSTATE：HY000

报错信息格式：服务器以--secure-auth 模式运行，但是'％s'@'％s'具有旧格式的密码；请更改密码为新格式。

ER_SERVER_IS_IN_SECURE_AUTH_MODE 在 8.0.15 之后被删除。
...
报错信息格式：在 SELECT＃％d 中解析了 SELECT＃％d 的字段或引用'％s％s％s％s％s％s'。

</br>

- 错误号：1277; 符号： ER_BAD_SLAVE_UNTIL_COND; SQLSTATE： HY000

报错信息格式：START REPLICA UNTIL 的参数不正确或参数组合。

</br>

- 错误号：1278; 符号： ER_MISSING_SKIP_SLAVE; SQLSTATE： HY000

报错信息格式：建议对 START REPLICA UNTIL 进行逐步复制时，建议使用--skip-slave-start；否则，如果意外从属的 mysqld 重新启动，将会遇到问题。

</br>

- 错误号：1279; 符号： ER_UNTIL_COND_IGNORED; SQLSTATE： HY000

报错信息格式：不要启动 SQL 线程，因此将忽略 UNTIL 选项。
...
新的查询缓存大小为％lu
ER_WARN_QC_RESIZE 在 8.0.2 之后被删除。
...
报错信息格式：列'％s'不能成为 FULLTEXT 索引的一部分。
...
报错信息格式： MySQL 以--skip-name-resolve 模式启动；您必须在没有此开关的情况下重新启动它，此授权才能起作用。
...
报错信息格式：第％ld 行的列'％s'中的 TIMESTAMP 值无效。
...
报错信息格式：％s（）的结果大于 max_allowed_packet（％ld）-被截断。
...
报错信息格式：PROCEDURE％s 无法在给定上下文中返回结果集。

</br>

- 错误号：1313; 符号： ER_SP_BADRETURN; SQLSTATE： 42000

报错信息格式：仅在 FUNCTION 中允许 RETURN。
...
报错信息格式：不推荐使用更新日志，并由二进制日志代替；SET SQL_LOG_UPDATE 已被忽略。
...
报错信息格式：不推荐使用更新日志，并由二进制日志代替；SET SQL_LOG_UPDATE 已转换为 SET SQL_LOG_BIN。
...
报错信息格式：％s％s 的参数数目不正确；预期％u，得到了％u。
...
报错信息格式：不能发布 EXPLAIN / SHOW；缺乏基础表的特权。
...
如果尝试在 INFORMATION_SCHEMA 实现为数据字典表视图的表上执行，则失败。
...
报错信息格式：不正确的％s 值：第％ld 行的列'％s'的'％s'。
...
报错信息格式： XAER_RMFAIL：当全局事务处于％s 状态时，该命令无法执行。
...
报错信息格式：无法授予 EXECUTE 和 ALTER ROUTINE 特权。
...
报错信息格式：例程％s 的 OUT 或 INOUT 参数％d 不是触发器之前的变量或 NEW 伪变量。
...
报错信息格式：无法从您发送到 GEOMETRY 字段的数据中获取几何对象。

</br>

- 错误号：1418; 符号： ER_BINLOG_UNSAFE_ROUTINE; SQLSTATE：HY000

报错信息格式：此函数的声明中没有 DETERMINISTIC，NO SQL 或 READS SQL DATA，并且启用了二进制日志记录（您可能希望使用不太安全的 log_bin_trust_function_creators 变量）。

</br>

- 错误号：1419; 符号： ER_BINLOG_CREATE_ROUTINE_NEED_SUPER; SQLSTATE：HY000

报错信息格式：您没有 SUPER 特权，并且启用了二进制日志记录（您可能想使用不太安全的 log_bin_trust_function_creators 变量）。
...
报错信息格式：对于 float（M，D），double（M，D）或小数点（M，D），M 必须大于等于 D（列'％s'）。
...
报错信息格式：线程堆栈溢出：使用了％ld 个字节堆栈中的％ld 个字节，并且需要％ld 个字节。
...
报错信息格式：您需要使用'％s'@'％s'定义程序创建视图的 SUPER 特权。
...
报错信息格式：例程％s 超出了递归限制％d（由 max_sp_recursion_depth 变量设置）。
...
报错信息格式：存储的功能不支持 AGGREGATE。

</br>

- 错误号：1461; 符号： ER_MAX_PREPARED_STMT_COUNT_REACHED; SQLSTATE：42000

报错信息格式：不能创建超过 max_prepared_stmt_count 条语句（当前值：％lu）。
...
报错信息格式：FIELDS TERMINATED 字符串的第一个字符不明确；请使用非可选和非空的字段。

</br>

- 错误号：1476; 符号： ER_FOREIGN_SERVER_EXISTS; SQLSTATE：HY000

报错信息格式：您尝试创建的外部服务器％s 已存在。
...
报错信息格式：语法错误：％s PARTITIONING 需要为每个分区定义 VALUES％s。

</br>

- 错误号：1480; 符号： ER_PARTITION_WRONG_VALUES_ERROR; SQLSTATE：HY000

报错信息格式：只有％s PARTITIONING 可以在分区定义中使用 VALUES％s。

</br>

- 错误号：1481; 符号： ER_PARTITION_MAXVALUE_ERROR; SQLSTATE：HY000

报错信息格式：MAXVALUE 只能在最后一个分区定义中使用。
...
报错信息格式：frm 文件中的分区信息与可写入 frm 文件的内容不一致。
...
报错信息格式：此版本的 MySQL 中不允许在分区中混合使用处理程序。
...
报错信息格式：只能将 RANGE / LIST 分区与 HASH / KEY 分区混合以进行子分区。
...
报错信息格式：％s 处理程序仅支持 VALUES 中的 32 位整数。
...
报错信息格式：未正确指定大小参数，无论是数字还是格式为 10M。

</br>

- 错误号：1532; 符号： ER_SIZE_OVERFLOW_ERROR; SQLSTATE： HY000
报错信息格式：大小数字正确，但我们不允许数字部分超过 20 亿。
...
为了使用 DML 语句获得最佳性能， InnoDB 需要在外键列上存在索引，以便父表 上的 UPDATE 和 DELETE 操作可以轻松检查子表中是否存在对应的行。

MySQL 的创建或在需要时自动下降这样的索引，作为副作用， 和 语句。

 CREATE TABLECREATE INDEXALTER TABLE
删除索引时，请 InnoDB 检查索引是否用于检查外键约束。

如果还有另一个可用于强制执行相同约束的索引，则仍然可以删除该索引。

InnoDB 防止您删除可以强制执行特定引用约束的最后一个索引。
...
报错信息格式：不建议使用语法'％s'，并将在 MySQL ％s 中将其删除。
...
报错信息格式：坚持表'％s'，条目'％s'，键％d 的外键约束将导致条目重复。
...
ER_TEMP_TABLE_PREVENTS_SWITCH_OUT_OF_RBR 在 8.0.12 之后被删除。
...
ER_DDL_LOG_ERROR 在 8.0.1 之后被删除。
...
报错信息格式：不允许使用小于 VALUES 的 NULL 值。
...
报错信息格式：ALTER TABLE 导致 auto_increment 重新排序，导致键“％s”的条目“％s”重复。
...
报错信息格式：事件执行时间已过去，并且设置了 ON COMPLETION NOT PRESERVE。
...
报错信息格式：由于 BINLOG_FORMAT = STATEMENT，所以使用语句格式将不安全的语句写入二进制日志。
...
ER_BINLOG_FATAL_ERROR 已在 8.0.11 中添加。
...
报错信息格式：类型％s 的 BINLOG 语句之前没有格式描述 BINLOG 语句。
...
报错信息格式：心跳周期的请求值是负值或超过了所允许的最大值（％s 秒）。
...
报错信息格式：mysql.ndb_replication 表的错误模式。
...
报错信息格式：SIGNAL / RESIGNAL 只能使用通过 SQLSTATE 定义的 CONDITION。
...
报错信息格式：请求的服务器标识％d 与从属启动选项--replicate-same-server-id 冲突。

</br>

- 错误号：1651; 符号： ER_QUERY_CACHE_DISABLED; SQLSTATE： HY000

报错信息格式：查询缓存已禁用；使用 query_cache_type = 1 重新启动服务器以启用它。

ER_QUERY_CACHE_DISABLED 在 8.0.2 之后被删除。
...
报错信息格式：无法执行语句：由于 BINLOG_FORMAT = ROW 并且至少一个表使用了仅限于基于语句的日志记录的存储引擎，因此无法写入二进制日志。
...
报错信息格式：无法执行语句：由于语句不安全，存储引擎仅限于基于语句的日志记录，并且 BINLOG_FORMAT = MIXED，因此无法写入二进制日志。
...
报错信息格式：无法执行语句：由于 BINLOG_FORMAT = STATEMENT 并且至少一个表使用了仅限于基于行的日志记录的存储引擎，因此无法写入二进制日志。
...
报错信息格式：无法执行语句：由于语句为行格式且 BINLOG_FORMAT = STATEMENT，因此无法写入二进制日志。
...
报错信息格式：该语句是不安全的，因为它使用了 LIMIT 子句。
...
报错信息格式：该语句不安全，因为它使用常规日志，慢查询日志或 Performance_schema 表。
...
报错信息格式：语句是不安全的，因为它调用插入到 AUTO_INCREMENT 列中的触发器或存储函数。
...
报错信息格式：语句是不安全的，因为它使用的 UDF 可能在从属服务器上未返回相同的值。
...
％s'的列％d 无法从类型'％s'转换为类型'％s'
ER_SLAVE_CONVERSION_FAILED 在 8.0.4 之后被删除。
...
报错信息格式：无效的 performance_schema 用法。
...
报错信息格式：无法更改存储的函数或触发器中的 binlog 直接标志。
...
报错信息格式：无法在存储的函数或触发器中更改 sql_log_bin。
...
报错信息格式：分区'％s'的 VALUES 值必须具有 INT 类型。
...
报错信息格式：SET PASSWORD 对用户通过插件进行身份验证没有意义。
...
报错信息格式：插件'％s'是 force_plus_permanent，无法卸载。

</br>

- 错误号：1703; 符号： ER_SLAVE_HEARTBEAT_VALUE_OUT_OF_RANGE_MIN; SQLSTATE：HY000

报错信息格式：心跳周期的请求值小于 1 毫秒。

将该值重置为 0，这意味着将有效禁用心跳。
...
报错信息格式：多行语句需要超过“ max_binlog_stmt_cache_size”个字节的存储空间；增加此 mysqld 变量，然后重试。
...
报错信息格式：INSERT IGNORE ... SELECT 是不安全的，因为 SELECT 检索行的顺序决定了忽略哪些行（如果有）。
...
报错信息格式：INSERT ... SELECT ... ON DUPLICATE KEY UPDATE 不安全，因为 SELECT 检索行的顺序决定了要更新的行（如果有）。
...
报错信息格式：REPLACE ... SELECT 是不安全的，因为 SELECT 检索行的顺序决定了要替换的行（如果有）。
...
报错信息格式：CREATE ... IGNORE SELECT 是不安全的，因为 SELECT 检索行的顺序决定了忽略哪些行（如果有）。
...
报错信息格式：CREATE ... REPLACE SELECT 是不安全的，因为 SELECT 检索行的顺序决定了要替换的行（如果有）。
...
报错信息格式：UPDATE IGNORE 是不安全的，因为更新行的顺序决定了忽略哪些（如果有）行。
...
报错信息格式：具有自动增量列的表上的 CREATE TABLE ... SELECT ...是不安全的，因为 SELECT 检索行的顺序决定了要插入的行（如果有）。
...
报错信息格式：INSERT ...在具有多个 UNIQUE KEY 的表上进行 DUPLICATE KEY UPDATE 不安全。
...
报错信息格式：主延迟请求的值％s 超过最大％u。

</br>

- 错误号：1730; 符号： ER_ONLY_FD_AND_RBR_EVENTS_ALLOWED_IN_BINLOG_STATEMENT; SQLSTATE：HY000

报错信息格式：BINLOG 语句中仅允许 Format_description_log_event 和行事件（但提供了％s）。
...
报错信息格式：选项 binlog_cache_size（％lu）大于 max_binlog_cache_size（％lu）; 设置 binlog_cache_size 等于 max_binlog_cache_size。
...
报错信息格式：由于字段'％s'上的类型或排序规则转换，无法在索引'％s'上使用％s 访问。
...
报错信息格式：选项 binlog_stmt_cache_size（％lu）大于 max_binlog_stmt_cache_size（％lu）；设置 binlog_stmt_cache_size 等于 max_binlog_stmt_cache_size。
...
报错信息格式：发送不带 SSL / TLS 的纯文本密码非常不安全。
...
报错信息格式：在主信息存储库中存储 MySQL 用户名或密码信息是不安全的，因此不建议这样做。

请考虑将 USER 和 PASSWORD 连接选项用于 START REPLICA；有关更多信息，请参见 MySQL 手册中的“ START REPLICA 语法”。
...
报错信息格式：仅从属 SQL 线程正在启动时，无法设置身份验证选项。
...
报错信息格式：当@@ SESSION.GTID_NEXT_LIST 为非 NULL 时，系统变量@@ SESSION.GTID_NEXT 不能为“ AUTOMATIC”。
...
报错信息格式：当 SOURCE_AUTO_POSITION 处于活动状态时，无法设置参数 SOURCE_LOG_FILE，SOURCE_LOG_POS，RELAY_LOG_FILE 和 RELAY_LOG_POS。
...
报错信息格式：由于@@ GLOBAL.GTID_MODE = OFF，无法执行 CHANGE REPLICATION SOURCE TO SOURCE_AUTO_POSITION = 1。
...
报错信息格式：GTID_MODE = ON 需要 ENFORCE_GTID_CONSISTENCY = ON。
...
报错信息格式：@@ GLOBAL.GTID_MODE = OFF 时，不能将@@ SESSION.GTID_NEXT 设置为 UUID：NUMBER。
...
报错信息格式：当@@ GLOBAL.GTID_MODE = ON 时，不能将@@ SESSION.GTID_NEXT 设置为匿名。
...
报错信息格式：@@ GLOBAL.GTID_MODE = OFF 时，不能将@@ SESSION.GTID_NEXT_LIST 设置为非 NULL 值。
...
报错信息格式：语句违反了 GTID 一致性：非事务表的更新只能在自动提交的语句或单语句事务中完成，而不能在与事务表的更新相同的语句中完成。
...
报错信息格式：语句违反了 GTID 一致性：CREATE TABLE ... SELECT。
...
报错信息格式：语句违反了 GTID 一致性：CREATE TEMPORARY TABLE 和 DROP TEMPORARY TABLE 只能在事务上下文之外执行。

在函数或触发器中也不允许使用这些语句，因为函数和触发器也被视为多语句事务。

ER_GTID_UNSAFE_CREATE_DROP_TEMPORARY_TABLE_IN_TRANSACTION 在 8.0.12 之后被删除。
...
报错信息格式：@@ GLOBAL.GTID_MODE 的值一次只能更改一次：OFF <-> OFF_PERMISSIVE <-> ON_PERMISSIVE <-> ON。
...
报错信息格式：@@ SESSION.GTID_NEXT 无法由拥有 GTID 的客户端更改。

客户拥有％s。

所有权在 COMMIT 或 ROLLBACK 上释放。
...
您必须至少设置--server-id 才能启用主服务器或从服务器。

其他错误消息可以在 MySQL 错误日志中找到。
...
报错信息格式：未知的 LOCK 类型'％s'。

</br>

- 错误号：1802; 符号： ER_MTS_CHANGE_MASTER_CANT_RUN_WITH_GAPS; SQLSTATE：HY000

报错信息格式：当从站由于错误而停止或在 MTS 模式下终止时，无法执行 CHANGE REPLICATION SOURCE。

考虑使用 RESET REPLICA 或 START REPLICA UNTIL。
...
报错信息格式：在并行执行模式下 SLAVE 错误出错后，无法恢复。
...
报错信息格式：ALTER TABLE％s 导入表空间失败，错误％lu：'％s'。
...
报错信息格式：您必须在执行此语句之前使用 ALTER USER 语句重置密码。
...
报错信息格式：列'％s'不能为空：在外键约束'％s'中需要 SET NULL。
...
报错信息格式：@@ SESSION.GTID_NEXT 设置为 GTID 时，必须在 COMMIT 或 ROLLBACK 之后将其显式设置为其他值。

请查看 GTID_NEXT 变量手册页以获取详细说明。

当前@@ SESSION.GTID_NEXT 为'％s'。

ER_GTID_NEXT_TYPE_UNDEFINED_GROUP 在 8.0.4 之后被删除。
...
ER_GTID_NEXT_TYPE_UNDEFINED_GTID 已在 8.0.11 中添加。
...
报错信息格式：@@ GLOBAL.GTID_PURGED 只能在@@ GLOBAL.GTID_EXECUTED 为空时设置。
...
报错信息格式：@@ GLOBAL.GTID_PURGED 只能在没有正在进行的事务时设置（甚至在其他客户端中也不能）。
...
报错信息格式：@@ GLOBAL.GTID_PURGED 已从“％s”更改为“％s”。
...
报错信息格式：@@ GLOBAL.GTID_EXECUTED 已从“％s”更改为“％s”。
...
报错信息格式：无法执行语句：由于 BINLOG_FORMAT = STATEMENT，并且已写入复制表和未复制表，因此无法写入二进制日志。
...
报错信息格式：分区特定的操作尚不支持 LOCK / ALGORITHM。
...
报错信息格式：当服务器运行@@ GLOBAL.GTID_MODE = ON 时，无法设置 sql_slave_skip_counter。

相反，对于您要跳过的每个事务，请生成一个与该事务具有相同 GTID 的空事务
...
报错信息格式：对象的长数据库名称和标识符导致路径长度超过％d 个字符。
...
报错信息格式：无法按此 SQL_MODE 的要求静默转换 NULL 值。
...
报错信息格式：在错误的分区％s 中找到一行。

</br>

- 错误号：1864; 符号： ER_MTS_EVENT_BIGGER_PENDING_JOBS_SIZE_MAX; SQLSTATE：HY000

报错信息格式：无法计划事件％s，中继日志名称％s，位置％s 到工作线程，因为它的大小％lu 超过 slave_pending_jobs_size_max 的％lu。
...
报错信息格式：未清除文件％s，因为％d 线程正在读取文件％s，仅清除了％d 文件中的％d。
...
报错信息格式：UPDATE 中的自动增量值与内部生成的值冲突。

</br>

- 错误号：1870; 符号： WARN_ON_BLOCKHOLE_IN_RBR; SQLSTATE：HY000

报错信息格式：未记录用于以行格式修改 BLACKHOLE 表的％s 语句的行事件。
...
报错信息格式：STOP REPLICA 命令执行未完成：从 SQL 线程收到停止信号，线程正忙，一旦当前任务完成， SQL 线程将停止。
...
报错信息格式：STOP REPLICA 命令执行未完成：从 IO 线程收到停止信号，线程正忙，一旦当前任务完成，IO 线程将停止。
...
报错信息格式：旧格式的 TIME / TIMESTAMP / DATETIME 列已升级为新格式。
...
报错信息格式：当 innodb_force_recovery> 0 时，不允许进行操作。
...
ER_GTID_UNSAFE_BINLOG_SPLITTABLE_STATEMENT_AND_GTID_GROUP 在 8.0.4 之后被删除。
...
ER_GTID_UNSAFE_BINLOG_SPLITTABLE_STATEMENT_AND_ASSIGNED_GTID 已在 8.0.11 中添加。
...
报错信息格式：使用主服务器的 SERVER_UUID，从服务器具有比主服务器更多的 GTID。

这可能表明二进制日志的末尾已被截断，或者最后一个二进制日志文件已丢失，例如，在 sync_binlog！= 1 的电源或磁盘故障之后。

主数据库可能已回滚或可能未回滚已复制的事务。

给奴隶 建议将主服务器已从从服务器回滚的所有事务复制到主服务器，和/或在主服务器上提交空事务，以解决已在主服务器上提交但未包含在 GTID_EXECUTED 中的事务。
...
报错信息格式：设置 named_pipe_full_access_group ='％s'是不安全的。

考虑使用成员较少的 Windows 组。

WARN_NAMED_PIPE_ACCESS_EVERYONE 已在 8.0.17 中添加。
...
如果确定主服务器正常，请在从服务器上手动运行此查询，然后使用 SET GLOBAL SQL_SLAVE_SKIP_COUNTER = 1 重新启动从服务器；否则，请重新启动该服务器。
...
报错信息格式：无法 DISCARD / IMPORT 表空间与临时表关联。
...
报错信息格式：仅 SELECT / UPDATE / INSERT / DELETE / REPLACE 支持 EXPLAIN FOR CONNECTION 命令。
...
报错信息格式：无法通过正在运行的从属 sql 线程执行此操作；首先运行 STOP REPLICA SQL_THREAD。

</br>

- 错误号：3018; 符号： ER_NO_FT_MATERIALIZED_SUBQUERY; SQLSTATE：HY000

报错信息格式：无法在实例化子查询上创建 FULLTEXT 索引。
...
报错信息格式：无法通过正在运行的从属 io 线程执行此操作；首先运行 STOP REPLICA IO_THREAD FOR CHANNEL'％s'。
...
在服务器重新启动或任何复制的 DROP 语句删除表之前，这些表将一直保持打开状态。

建议等待直到 slave_open_temp_tables = 0。
...
报错信息格式：使用 SOURCE_LOG_FILE 子句但没有 SOURCE_LOG_POS 子句的 CHANGE REPLICATION SOURCE TO 可能并不安全。
...
报错信息格式：ORDER BY 的表达式＃％u 包含聚合函数，并应用于 UNION。

</br>

- 错误号：3029; 符号： ER_AGGREGATE_ORDER_NON_AGG_QUERY; SQLSTATE：HY000

报错信息格式：ORDER BY 的表达式＃％u 包含聚合函数，并应用于非聚合查询的结果。

</br>

- 错误号：3030; 符号： ER_SLAVE_WORKER_STOPPED_PREVIOUS_THD_ERROR; SQLSTATE：HY000

报错信息格式：当启用 slave-preserve-commit-order 时，至少一个先前的工作程序遇到错误后，从属工作程序已停止。
...
报错信息格式：％s 不支持 slave_preserve_commit_order。
...
报错信息格式：二进制几何函数％s 给出了两个不同条纹的几何：％u 和％u，它们应该是相同的。
...
报错信息格式：几何转弯信息计算错误：几何数据在功能％s 中无效。
...
报错信息格式：为复制用户提供的密码超过了 32 个字符的最大长度。
...
当 metdata 锁定子系统检测到尝试使用获取命名锁的死锁时，将返回此错误 GET_LOCK。
...
报错信息格式：ORDER BY 子句的表达式＃％u 不在 SELECT 列表中，引用的列'％s'不在 SELECT 列表中；这与％s 不兼容。

</br>

- 错误号：3066; 符号： ER_AGGREGATE_IN_ORDER_NOT_SELECT; SQLSTATE：HY000

报错信息格式：ORDER BY 子句的表达式＃％u 不在 SELECT 列表中，包含聚合函数；这与％s 不兼容。
...
报错信息格式：要具有多个通道，存储库不能为 FILE 类型；请检查存储库配置并将其转换为 TABLE。
...
报错信息格式：无法通过正在运行的复制线程执行此操作；首先运行 STOP REPLICA FOR CHANNEL'％s'。

</br>

- 错误号：3082; 符号： ER_SLAVE_CHANNEL_NOT_RUNNING; SQLSTATE：HY000

报错信息格式：此操作需要运行复制线程；配置从站并运行 START REPLICA FOR CHANNEL'％s'。
...
报错信息格式：无法通过正在运行的从属 sql 线程执行此操作；首先运行 STOP REPLICA SQL_THREAD FOR CHANNEL'％s'。
...
报错信息格式：当 sql_slave_skip_counter> 0 时，不允许使用'START REPLICA [SQL_THREAD]'启动多个 SQL 线程。

sql_slave_skip_counter 的值一次只能由一个 SQL 线程使用。

请使用“ START REPLICA [SQL_THREAD] FOR CHANNEL”启动将使用 sql_slave_skip_counter 值的 SQL 线程。
...
报错信息格式：％s 的表达式＃％u 不在 GROUP BY 子句中，并且包含未聚合的列'％s'，该列在功能上不依赖于 GROUP BY 子句中的列；这与 sql_mode = only_full_group_by 不兼容。

</br>

- 错误号：3088; 符号： ER_MIX_OF_GROUP_FUNC_AND_FIELDS_V2; SQLSTATE：HY000

报错信息格式：在没有 GROUP BY 的聚合查询中，％s 的表达式＃％u 包含非聚合列'％s'；这与 sql_mode = only_full_group_by 不兼容。
...
报错信息格式：DROP DATABASE 失败；某些表可能已删除，但数据库目录仍然保留。

GTID 尚未添加到 GTID_EXECUTED，并且该语句未写入二进制日志。

对此进行如下修复：（1）从数据库目录％s 中删除所有文件；（2）SET GTID_NEXT ='％s'; （3）删除数据库'％s'。
...
报错信息格式：由于该组已经在运行，因此 START GROUP_REPLICATION 命令失败。
...
报错信息格式：由于应用程序模块无法启动，因此 START GROUP_REPLICATION 命令失败。
...
报错信息格式：STOP GROUP_REPLICATION 命令执行未完成：应用程序线程在繁忙时收到停止信号。
...
报错信息格式：START GROUP_REPLICATION 命令失败，因为初始化组通信层时出错。
...
报错信息格式：START GROUP_REPLICATION 命令失败，因为加入通信组时出现错误。
...
报错信息格式：如果没有二进制日志格式为 ROW，则无法更改变量％s 的值。

transaction_write_set_extraction 选项值已设置，binlog_format 但未 设置 ROW。
...
报错信息格式：虚拟列的 INPLACE ADD 或 DROP 不能与其他 ALTER TABLE 操作结合使用。

</br>

- 错误号：3104; 符号： ER_WRONG_FK_OPTION_FOR_GENERATED_COLUMN; SQLSTATE：HY000

报错信息格式：无法在生成的列上使用％s 子句定义外键。
...
报错信息格式：“％s”功能不可用；您需要删除“％s”或使用由“％s”构建的 MySQL 。

</br>

- 错误号：3111; 符号： ER_CANT_SET_GTID_MODE; SQLSTATE： HY000

报错信息格式：因为％s，所以不允许使用 SET @@ GLOBAL.GTID_MODE =％s。
...
报错信息格式：复制接收器线程％s 无法在 AUTO_POSITION 模式下启动：此服务器使用@@ GLOBAL.GTID_MODE = OFF。
...
报错信息格式：无法设置 ENFORCE_GTID_CONSISTENCY = ON，因为正在进行的事务违反了 GTID 一致性。
...
该帐户已用 CREATE USER ... ACCOUNT LOCK 或 锁定 ALTER USER ... ACCOUNT LOCK。
...
报错信息格式：不支援的 MAX_EXECUTION_TIME。

</br>

- 错误号：3125; 符号： ER_WARN_UNSUPPORTED_MAX_EXECUTION_TIME; SQLSTATE：HY000

报错信息格式：MAX_EXECUTION_TIME 提示仅受顶级独立 SELECT 语句支持。

该 MAX_EXECUTION_TIME 优化提示只为支持 SELECT 声明。
...
这是一个 mysql 内部系统表，用于存储已提交事务的 GTID。
...
锁定服务名称指定为 NULL，空字符串或长度超过 64 个字符的字符串。

命名空间和锁名称必须为非 NULL，非空，并且长度不能超过 64 个字符。
...
报错信息格式：参数％s 超出了函数％s 中几何图形（％lu）的最大点数。
...
报错信息格式：“ NO_ZERO_DATE”，“ NO_ZERO_IN_DATE”和“ ERROR_FOR_DIVISION_BY_ZERO” sql 模式应与严格模式一起使用。
...
* s
客户端已将其 version_tokens_session 系统变量设置 为需要服务器匹配的令牌列表，但是服务器令牌列表具有至少一个匹配的令牌名称，该名称的值不同于客户端所需的值。
...
客户端已将其 version_tokens_session 系统变量设置 为需要服务器匹配的令牌列表，但是服务器令牌列表至少缺少这些令牌之一。
...
报错信息格式：无效的 JSON 文本：列“％s”中位于值％u 处的“％s”。
...
报错信息格式：函数％s 中的参数％u 中的无效 JSON 文本：位置％u。
...
报错信息格式：无法从具有 CHARACTER SET'％s'的字符串创建 JSON 值。
...
报错信息格式：提供给函数％s 的无效 JSON 字符数据：'％s'; utf8 是必需的。
...
报错信息格式：函数％s 的参数％u 中的 JSON 数据的数据类型无效； JSON 字符串或 JSON 类型是必需的。
...
报错信息格式： JSON 列'％s'仅支持通过指定 JSON 路径上的生成列进行索引。
...
报错信息格式：--require_secure_transport = ON 时，禁止使用不安全的传输进行连接。

使用 require_secure_transport 系统变量，客户端只能使用安全传输进行连接。

合格连接是使用 SSL，Unix 套接字文件或共享内存的连接。
...
报错信息格式：未配置安全传输（SSL 或共享内存），无法设置--require_secure_transport = ON。

该 require_secure_transport 如果服务器不支持至少一个安全可靠的运输系统变量无法启用。

使用必需的 SSL 密钥/证书配置服务器以启用 SSL 连接，或启用 shared_memory 系统变量以启用共享内存连接。
...
报错信息格式：存储引擎％s 已禁用（不允许创建表）。

试图使用 disabled_storage_engines 系统变量值中列出的存储引擎来创建表或表空间，或将现有表或表空间更改为此类引擎。
...
报错信息格式：客户端拥有 GTID％s 的所有权。

因此，WAIT_FOR_EXECUTED_GTID_SET 无法等待此 GTID。
...
报错信息格式：请不要用 XA 事务修改％s 表。

这是一个内部系统表，用于存储已提交事务的 GTID。

尽管修改它可能导致 GTID 状态不一致，但是如果需要，您可以使用非 XA 事务进行修改。
...
报错信息格式：由于％s，不允许使用 RESET BINARY LOGS AND GTIDS。
...
报错信息格式：由于该组已有 9 个成员，因此 START GROUP_REPLICATION 命令失败。
...
报错信息格式：功能％s 失败，原因是：％s。

ER_KEYRING_AWS_UDF_AWS_KMS_ERROR 在 8.0.2 中添加。
...
报错信息格式：语句不安全，因为它在 XA 事务中使用。

使用语句复制时，并发 XA 事务可能在从属服务器上死锁。

ER_BINLOG_UNSAFE_XA 在 8.0.2 中添加。
...
ER_KEYRING_MIGRATION_FAILURE 在 8.0.4 中添加。
...
报错信息格式：访问被拒绝；您需要％s 权限才能执行此操作。

ER_KEYRING_ACCESS_DENIED_ERROR 在 8.0.4 中添加。
...
ER_KEYRING_MIGRATION_STATUS 在 8.0.4 中添加。
...
ER_AUDIT_LOG_UDF_READ_INVALID_MAX_ARRAY_LENGTH_ARG_VALUE 已在 8.0.11 中添加。
...
报错信息格式：ROW_FORMAT = COMPRESSED 或 KEY_BLOCK_SIZE 不允许使用 CREATE TEMPORARY TABLE。
...
报错信息格式：由于来自 SE 的以下错误，ACL 操作失败：错误代码％d-％s。

</br>

- 错误号：3502; 符号： ER_UNSUPPORTED_INDEX_ALGORITHM; SQLSTATE：HY000

报错信息格式：此存储引擎不支持％s 索引算法，而是使用了存储引擎默认值。
...
报错信息格式：聚合的按位函数不能接受超过 511 个字节的参数；考虑使用 SUBSTRING（）函数。
...
报错信息格式：SRID％u 的空间参考系统定义未指定强制性％s（EPSG％u）投影参数。
...
报错信息格式：函数％s 仅为笛卡尔空间参考系统定义，但其参数之一在 SRID％u 中，而不是笛卡尔。
...
报错信息格式：仅为笛卡尔空间参考系统定义函数％s，但其参数之一在 SRID％u 中，尚未定义。
...
报错信息格式：无法获取指定的 URN：％s 中的模式“％s”的方案加载服务实现。
...
报错信息格式：Persistent Dynamic Loader 已用于卸载组件'％s'，但之前并未用于加载该组件。
...
报错信息格式：mysql.component 表丢失或定义不正确。
...
报错信息格式：在组中多次指定了具有指定的 URN：'％s'的组件。
...
报错信息格式：@@ GLOBAL.GTID_PURGED 无法更改：％s。
...
ER_NO_SYSTEM_TABLE_ACCESS_FOR_DICTIONARY_TABLE 在 8.0.1 中添加。
...
ER_NO_SYSTEM_TABLE_ACCESS_FOR_SYSTEM_TABLE 在 8.0.1 中添加。
...
ER_NO_SYSTEM_TABLE_ACCESS_FOR_TABLE 在 8.0.1 中添加。
...
报错信息格式：函数％s 中的无效选项键'％s'。

ER_INVALID_OPTION_KEY 在 8.0.1 中添加。
...
ER_INVALID_OPTION_VALUE 在 8.0.1 中添加。
...
报错信息格式：字符串'％s'在函数％s 中不是有效的键％c 值对。

ER_INVALID_OPTION_KEY_VALUE_PAIR 在 8.0.1 中添加。
...
报错信息格式：函数％s 中的 options 参数以无效字符'％c'开头。

ER_INVALID_OPTION_START_CHARACTER 在 8.0.1 中添加。
...
报错信息格式：函数％s 中的 options 参数以无效字符'％c'结尾。

ER_INVALID_OPTION_END_CHARACTER 在 8.0.1 中添加。
...
报错信息格式：函数％s 中的 options 参数包含无效的字符序列'％s'。

ER_INVALID_OPTION_CHARACTERS 在 8.0.1 中添加。
...
ER_DUPLICATE_OPTION_KEY 在 8.0.1 中添加。
...
ER_WARN_SRS_NOT_FOUND_AXIS_ORDER 在 8.0.1 中添加。
...
ER_NO_ACCESS_TO_NATIVE_FCT 在 8.0.1 中添加。
...
ER_RESET_MASTER_TO_VALUE_OUT_OF_RANGE 在 8.0.1 中添加。
...
ER_UNRESOLVED_TABLE_LOCK 在 8.0.1 中添加。
...
报错信息格式：表％s 出现在多个锁定子句中。

ER_DUPLICATE_TABLE_LOCK 在 8.0.1 中添加。
...
ER_BINLOG_UNSAFE_SKIP_LOCKED 在 8.0.1 中添加。
...
报错信息格式：语句不安全，因为它使用 NOWAIT。

命令是成功还是失败是不确定的。

ER_BINLOG_UNSAFE_NOWAIT 在 8.0.1 中添加。
...
报错信息格式：语句中止，因为无法立即获取锁并且设置了 NOWAIT。

ER_LOCK_NOWAIT 在 8.0.1 中添加。
...
报错信息格式：递归公用表表达式'％s'应该包含 UNION。

ER_CTE_RECURSIVE_REQUIRES_UNION 在 8.0.1 中添加。
...
ER_CTE_RECURSIVE_REQUIRES_NONRECURSIVE_FIRST 在 8.0.1 中添加。
...
ER_CTE_RECURSIVE_FORBIDS_AGGREGATION 在 8.0.1 中添加。
...
报错信息格式：在递归公用表表达式'％s'的递归查询块中，递归表既不能在 LEFT JOIN 的正确参数中，也不能被强制为非第一个具有连接顺序提示的表。

ER_CTE_RECURSIVE_FORBIDDEN_JOIN_ORDER 在 8.0.1 中添加。
...
ER_CTE_RECURSIVE_REQUIRES_SINGLE_REFERENCE 在 8.0.1 中添加。
...
ER_SWITCH_TMP_ENGINE 在 8.0.1 中添加。
...
ER_WINDOW_NO_SUCH_WINDOW 在 8.0.2 中添加。
...
ER_WINDOW_CIRCULARITY_IN_WINDOW_GRAPH 在 8.0.2 中添加。
...
ER_WINDOW_NO_CHILD_PARTITIONING 在 8.0.2 中添加。
...
ER_WINDOW_NO_INHERIT_FRAME 在 8.0.2 中添加。
...
报错信息格式：窗口'％s'不能继承'％s'，因为它们都包含 ORDER BY 子句。

ER_WINDOW_NO_REDEFINE_ORDER_BY 在 8.0.2 中添加。
...
ER_WINDOW_FRAME_START_ILLEGAL 在 8.0.2 中添加。
...
ER_WINDOW_FRAME_END_ILLEGAL 在 8.0.2 中添加。
...
报错信息格式：窗口'％s'：帧的开始或结束为负，NULL 或非整数类型。

ER_WINDOW_FRAME_ILLEGAL 在 8.0.2 中添加。
...
报错信息格式：具有范围 N 前/后帧的窗口'％s'要求仅一个数字或时间类型的 ORDER BY 表达式。

ER_WINDOW_RANGE_FRAME_ORDER_TYPE 在 8.0.2 中添加。
...
报错信息格式：带有范围框架的窗口'％s'具有日期时间类型的 ORDER BY 表达式。

仅允许 INTERVAL 界限值。

ER_WINDOW_RANGE_FRAME_TEMPORAL_TYPE 在 8.0.2 中添加。
...
报错信息格式：带有范围框架的窗口'％s'具有数字类型的 ORDER BY 表达式，不允许 INTERVAL 绑定值。

ER_WINDOW_RANGE_FRAME_NUMERIC_TYPE 在 8.0.2 中添加。
...
ER_WINDOW_RANGE_BOUND_NOT_CONSTANT 在 8.0.2 中添加。
...
ER_WINDOW_DUPLICATE_NAME 在 8.0.2 中添加。
...
报错信息格式：窗口'％s'：ORDER BY 或 PARTITION BY 使用不支持的旧位置指示，请使用表达式。

ER_WINDOW_ILLEGAL_ORDER_BY 在 8.0.2 中添加。
...
ER_WINDOW_INVALID_WINDOW_FUNC_USE 在 8.0.2 中添加。
...
ER_WINDOW_INVALID_WINDOW_FUNC_ALIAS_USE 在 8.0.2 中添加。
...
ER_WINDOW_NESTED_WINDOW_FUNC_USE_IN_WINDOW_SPEC 在 8.0.2 中添加。
...
报错信息格式：窗口'％s'：INTERVAL 仅可用于 RANGE 帧。

ER_WINDOW_ROWS_INTERVAL_USE 在 8.0.2 中添加。
...
报错信息格式：窗口功能不允许带有 GROUP BY 的 ASC 或 DESC；将 ASC 或 DESC 放入 ORDER BY。

ER_WINDOW_NO_GROUP_ORDER 在 8.0.2 中新增，在 8.0.12 之后删除。
...
ER_WINDOW_NO_GROUP_ORDER_UNUSED 已在 8.0.13 中添加。
...
报错信息格式：要获取有关窗口函数的信息，请使用 EXPLAIN FORMAT = JSON 。

ER_WINDOW_EXPLAIN_JSON 在 8.0.2 中添加。
...
报错信息格式：窗口函数'％s'忽略窗口'％s'的 frame 子句，并在整个分区上聚合。

ER_WINDOW_FUNCTION_IGNORES_FRAME 在 8.0.2 中添加。
...
ER_WINDOW_SE_NOT_ACCEPTABLE 在 8.0.2 中新增，在 8.0.3 之后删除。
...
ER_WL9236_NOW_UNUSED 在 8.0.4 中添加。
...
报错信息格式：函数％s 的参数过多：％lu; 允许的最大值为％s。

ER_INVALID_NO_OF_ARGS 在 8.0.1 中添加。
...
报错信息格式：GROUPING 函数的参数＃％u 不在 GROUP BY 中。

ER_FIELD_IN_GROUPING_NOT_GROUP_BY 在 8.0.1 中添加。
...
ER_TOO_LONG_TABLESPACE_COMMENT 在 8.0.1 中添加。
...
ER_ENGINE_CANT_DROP_TABLE 在 8.0.1 中添加。
...
使用 DROP TABLE IF EXISTS 将其从数据字典中删除。

ER_ENGINE_CANT_DROP_MISSING_TABLE 在 8.0.1 中添加。
...
ER_TABLESPACE_DUP_FILENAME 在 8.0.1 中添加。
...
报错信息格式：没有 SDI 文件与模式'％s'相匹配。

ER_IMP_NO_FILES_MATCHED 在 8.0.1 中添加。
...
报错信息格式：在 SDI 中引用的架构'％s'不存在。

ER_IMP_SCHEMA_DOES_NOT_EXIST 在 8.0.1 中添加。
...
报错信息格式：在 SDI 中引用的表'％s。

％s'已经存在。

ER_IMP_TABLE_ALREADY_EXISTS 在 8.0.1 中添加。
...
报错信息格式：导入的 mysqld_version（％llu）与当前版本（％llu）不兼容。

ER_IMP_INCOMPATIBLE_MYSQLD_VERSION 在 8.0.1 中添加。
...
报错信息格式：导入的 dd 版本（％u）与当前版本（％u）不兼容。

ER_IMP_INCOMPATIBLE_DD_VERSION 在 8.0.1 中添加。
...
报错信息格式：导入的 sdi 版本（％llu）与当前版本（％llu）不兼容。

ER_IMP_INCOMPATIBLE_SDI_VERSION 在 8.0.1 中添加。
...
ER_WARN_INVALID_HINT 在 8.0.1 中添加。
...
报错信息格式：变量％s 在持久配置文件中不存在。

ER_VAR_DOES_NOT_EXIST 在 8.0.1 中添加。
...
报错信息格式：经度％f 在功能％s 中超出范围。

它必须在（％f，％f]之内。

ER_LONGITUDE_OUT_OF_RANGE 在 8.0.1 中添加。
...
报错信息格式：纬度％f 在功能％s 中超出范围。

它必须在[％f，％f]之内。

ER_LATITUDE_OUT_OF_RANGE 在 8.0.1 中添加。
...
ER_NOT_IMPLEMENTED_FOR_GEOGRAPHIC_SRS 在 8.0.1 中添加。
...
报错信息格式：为％s 指定了非法特权级别。

ER_ILLEGAL_PRIVILEGE_LEVEL 在 8.0.1 中添加。
...
报错信息格式：无法访问系统视图 INFORMATION_SCHEMA。

'％s'被拒绝。

ER_NO_SYSTEM_VIEW_ACCESS 在 8.0.2 中添加。
...
ER_COMPONENT_FILTER_FLABBERGASTED 在 8.0.2 中添加。
...
ER_PART_EXPR_TOO_LONG 在 8.0.2 中添加。
...
报错信息格式：DROP FUNCTION 无法删除动态注册的用户定义函数。

ER_UDF_DROP_DYNAMICALLY_REGISTERED 在 8.0.2 中添加。
...
'％s'
ER_UNABLE_TO_STORE_COLUMN_STATISTICS 在 8.0.2 中添加。
...
'％s'
ER_UNABLE_TO_UPDATE_COLUMN_STATISTICS 在 8.0.2 中添加。
...
'％s'
ER_UNABLE_TO_DROP_COLUMN_STATISTICS 在 8.0.2 中添加。
...
'％s'
ER_UNABLE_TO_BUILD_HISTOGRAM 在 8.0.2 中添加。
...
报错信息格式：角色％s 是强制性角色，不能撤消或删除。

可以通过将角色标识符从全局变量 strict_roles 中排除来解除限制。

ER_MANDATORY_ROLE 在 8.0.2 中添加。
...
ER_MISSING_TABLESPACE_FILE 在 8.0.3 中添加。
...
ER_PERSIST_ONLY_ACCESS_DENIED_ERROR 在 8.0.3 中添加。
...
报错信息格式：您需要命令'％s'的 SUPER 特权。

ER_CMD_NEED_SUPER 在 8.0.3 中添加。
...
ER_PATH_IN_DATADIR 在 8.0.3 中添加。
...
报错信息格式：在操作过程中执行并发 DDL 。

请再试一遍。

ER_DDL_IN_PROGRESS 在 8.0.3 中新增，在 8.0.16 之后删除了。
...
报错信息格式：在克隆操作期间执行了并行 DDL 。

请再试一遍。

ER_CLONE_DDL_IN_PROGRESS 已在 8.0.17 中添加。
...
ER_TOO_MANY_CONCURRENT_CLONES 在 8.0.3 中新增，在 8.0.16 之后删除了。
...
ER_CLONE_TOO_MANY_CONCURRENT_CLONES 已在 8.0.17 中添加。
...
报错信息格式：事务％s 中的表不符合外部插件的要求。

ER_APPLIER_LOG_EVENT_VALIDATION_ERROR 在 8.0.3 中添加。
...
尝试将@@ cte_max_recursion_depth 增加到更大的值。

ER_CTE_MAX_RECURSION_DEPTH 在 8.0.3 中添加。
...
报错信息格式：无法使用 SET_VAR 提示设置变量％s。

ER_NOT_HINT_UPDATABLE_VARIABLE 在 8.0.3 中添加。
...
ER_CREDENTIALS_CONTRADICT_TO_HISTORY 在 8.0.3 中添加。
...
报错信息格式：用户'％s'@'％s'忽略了非零密码历史记录子句，因为其身份验证插件％s 不支持密码历史记录。

ER_WARNING_PASSWORD_HISTORY_CLAUSES_VOID 在 8.0.3 中添加。
...
ER_CLIENT_DOES_NOT_SUPPORT 在 8.0.3 中添加。
...
报错信息格式：表空间'％s'被跳过，因为其定义被并发 DDL 语句修改。

ER_I_S_SKIPPED_TABLESPACE 在 8.0.3 中添加。
...
ER_TABLESPACE_ENGINE_MISMATCH 在 8.0.3 中添加。
...
报错信息格式：几何的 SRID 与列'％s'的 SRID 不匹配。

几何的 SRID 为％lu，但列的 SRID 为％lu。

考虑更改几何的 SRID 或列的 SRID 属性。

ER_WRONG_SRID_FOR_COLUMN 在 8.0.3 中添加。
...
报错信息格式：因为列上有空间索引，所以无法更改列'％s'上的 SRID 规范。

请先更改空间索引，然后再更改 SRID 规范。

ER_CANNOT_ALTER_SRID_DUE_TO_INDEX 在 8.0.3 中添加。
...
报错信息格式：％s 时，选项 binlog_row_value_options =％s 将被忽略，更新将以完整格式写入二进制日志。

ER_WARN_BINLOG_PARTIAL_UPDATES_DISABLED 在 8.0.3 中添加。
...
报错信息格式：％s 时，选项 log_bin_use_v1_row_events = 1 将被忽略，并且行事件将以新格式写入二进制日志。

ER_WARN_BINLOG_V1_ROW_EVENTS_DISABLED 在 8.0.3 中添加。
...
报错信息格式：％s 时，选项 binlog_row_value_options =％s 将仅用于残像。

完整值将写入前映像中，因此由于 binlog_row_value_options 而导致的磁盘空间节省限制为小于 50 %%。

ER_WARN_BINLOG_PARTIAL_UPDATES_SUGGESTS_PARTIAL_IMAGES 在 8.0.3 中添加。
...
ER_COULD_NOT_APPLY_JSON_DIFF 在 8.0.3 中添加。
...
* s，列％s 的 JSON diff 损坏。

ER_CORRUPTED_JSON_DIFF 在 8.0.3 中添加。
...
ER_RESOURCE_GROUP_EXISTS 在 8.0.3 中添加。
...
ER_RESOURCE_GROUP_NOT_EXISTS 在 8.0.3 中添加。
...
ER_INVALID_VCPU_ID 在 8.0.3 中添加。
...
报错信息格式：无效的 VCPU 范围％u-％u。

ER_INVALID_VCPU_RANGE 在 8.0.3 中添加。
...
报错信息格式：％s 资源组％s 的无效线程优先级值％d。

允许的范围是[％d，％d]。

ER_INVALID_THREAD_PRIORITY 在 8.0.3 中添加。
...
报错信息格式：％s 上不允许进行％s 操作。

ER_DISALLOWED_OPERATION 在 8.0.3 中添加。
...
ER_RESOURCE_GROUP_BUSY 在 8.0.3 中添加。
...
ER_RESOURCE_GROUP_DISABLED 在 8.0.3 中添加。
...
ER_FEATURE_UNSUPPORTED 在 8.0.3 中添加。
...
ER_ATTRIBUTE_IGNORED 在 8.0.3 中添加。
...
ER_INVALID_THREAD_ID 在 8.0.3 中添加。
...
报错信息格式：无法将资源组％s 与线程 ID（％llu）。

（％s）绑定。

ER_RESOURCE_GROUP_BIND_FAILED 在 8.0.3 中添加。
...
报错信息格式：选项 FORCE 无效，因为未指定 DISABLE 选项。

ER_INVALID_USE_OF_FORCE_OPTION 在 8.0.3 中添加。
...
％s
ER_GROUP_REPLICATION_COMMAND_FAILURE 在 8.0.4 中添加。
...
ER_SDI_OPERATION_FAILED 在 8.0.3 中添加。
...
报错信息格式：缺少 JSON_TABLE 列'％s'的值。

ER_MISSING_JSON_TABLE_VALUE 在 8.0.4 中添加。
...
报错信息格式：无法在标量 JSON_TABLE 列'％s'中存储数组或对象。

ER_WRONG_JSON_TABLE_VALUE 在 8.0.4 中添加。
...
ER_TF_MUST_HAVE_ALIAS 在 8.0.4 中添加。
...
报错信息格式：INNER 或 LEFT JOIN 必须用于'％s'所做的 LATERAL 引用。

ER_TF_FORBIDDEN_JOIN_TYPE 在 8.0.4 中添加。
...
报错信息格式：值超出 JSON_TABLE 的列'％s'的范围。

ER_JT_VALUE_OUT_OF_RANGE 在 8.0.4 中添加。
...
报错信息格式：在 JSON_TABLE'％s'中发现了超过受支持的％u 嵌套路径。

ER_JT_MAX_NESTED_PATH 在 8.0.4 中添加。
...
* s 不支持密码过期
ER_PASSWORD_EXPIRATION_NOT_SUPPORTED_BY_AUTH_METHOD 在 8.0.4 中添加。
...
报错信息格式：提供给函数％s 的无效 GeoJSON 数据：必须在顶级对象中指定成员'crs'。

ER_INVALID_GEOJSON_CRS_NOT_TOP_LEVEL 在 8.0.4 中添加。
...
ER_BAD_NULL_ERROR_NOT_IGNORED 在 8.0.4 中添加。
...
报错信息格式：查询优化器将不使用列'％s'上的空间索引，因为该列没有 SRID 属性。

考虑将 SRID 属性添加到该列。

WARN_USELESS_SPATIAL_INDEX 在 8.0.4 中添加。
...
ER_DISK_FULL_NOWAIT 在 8.0.4 中添加。
...
ER_PARSE_ERROR_IN_DIGEST_FN 在 8.0.4 中添加。
...
ER_UNDISCLOSED_PARSE_ERROR_IN_DIGEST_FN 在 8.0.4 中添加。
...
ER_SCHEMA_DIR_EXISTS 在 8.0.4 中添加。
...
ER_SCHEMA_DIR_MISSING 在 8.0.4 中添加。
...
ER_SCHEMA_DIR_CREATE_FAILED 在 8.0.4 中添加。
...
ER_SCHEMA_DIR_UNKNOWN 在 8.0.4 中添加。
...
报错信息格式：仅为 SRID 0 和 SRID 4326 定义了功能％s。

ER_ONLY_IMPLEMENTED_FOR_SRID_0_AND_4326 在 8.0.4 中添加。
...
报错信息格式：选项 expire_logs_days 和 binlog_expire_logs_seconds 不能一起使用。

请使用 binlog_expire_logs_seconds 设置到期时间（不建议使用 expire_logs_days）
ER_BINLOG_EXPIRE_LOG_DAYS_AND_SECS_USED_TOGETHER 已在 8.0.11 中添加。
...
ER_REGEXP_STRING_NOT_TERMINATED 在 8.0.4 中新增，在 8.0.4 之后删除了。
...
ER_REGEXP_BUFFER_OVERFLOW 在 8.0.4 中添加。
...
ER_REGEXP_ILLEGAL_ARGUMENT 在 8.0.4 中添加。
...
ER_REGEXP_INDEX_OUTOFBOUNDS_ERROR 在 8.0.4 中添加。
...
ER_REGEXP_INTERNAL_ERROR 在 8.0.4 中添加。
...
报错信息格式：第％u 行的正则表达式的语法错误，字符％u。

ER_REGEXP_RULE_SYNTAX 在 8.0.4 中添加。
...
ER_REGEXP_BAD_ESCAPE_SEQUENCE 在 8.0.4 中添加。
...
ER_REGEXP_UNIMPLEMENTED 在 8.0.4 中添加。
...
ER_REGEXP_MISMATCHED_PAREN 在 8.0.4 中添加。
...
ER_REGEXP_BAD_INTERVAL 在 8.0.4 中添加。
...
ER_REGEXP_MAX_LT_MIN 在 8.0.4 中添加。
...
ER_REGEXP_INVALID_BACK_REF 在 8.0.4 中添加。
...
ER_REGEXP_LOOK_BEHIND_LIMIT 在 8.0.4 中添加。
...
ER_REGEXP_MISSING_CLOSE_BRACKET 在 8.0.4 中添加。
...
报错信息格式：正则表达式包含[xy]字符范围，其中 x 在 y 之后。

ER_REGEXP_INVALID_RANGE 在 8.0.4 中添加。
...
ER_REGEXP_STACK_OVERFLOW 在 8.0.4 中添加。
...
ER_REGEXP_TIME_OUT 在 8.0.4 中添加。
...
ER_REGEXP_PATTERN_TOO_BIG 在 8.0.4 中添加。
...
报错信息格式：％s 的值在“％s”或附近引起混淆。

语法可能有误，可能未安装组件，或者列出了不支持实例的组件多次。

ER_CANT_SET_ERROR_LOG_SERVICE 在 8.0.4 中添加。
...
报错信息格式：设置一个空的％s 管道将禁用错误日志记录！。

ER_EMPTY_PIPELINE_FOR_ERROR_LOG_SERVICE 在 8.0.4 中添加。
...
ER_COMPONENT_FILTER_DIAGNOSTICS 在 8.0.4 中添加。
...
报错信息格式：ignore-builtin-innodb 将被忽略，并将在以后的版本中删除。

ER_INNODB_CANNOT_BE_IGNORED 在 8.0.2 中新增，在 8.0.2 之后删除。
...
ER_NOT_IMPLEMENTED_FOR_CARTESIAN_SRS 在 8.0.4 中添加。
...
ER_NOT_IMPLEMENTED_FOR_PROJECTED_SRS 在 8.0.4 中添加。
...
报错信息格式：提供给功能％s 的无效半径：半径必须大于零。

ER_NONPOSITIVE_RADIUS 在 8.0.4 中添加。
...
ER_RESTART_SERVER_FAILED 在 8.0.4 中添加。
...
ER_SRS_MISSING_MANDATORY_ATTRIBUTE 在 8.0.4 中添加。
...
ER_SRS_MULTIPLE_ATTRIBUTE_DEFINITIONS 在 8.0.4 中添加。
...
ER_SRS_NAME_CANT_BE_EMPTY_OR_WHITESPACE 在 8.0.4 中添加。
...
ER_SRS_ORGANIZATION_CANT_BE_EMPTY_OR_WHITESPACE 在 8.0.4 中添加。
...
报错信息格式：已经有一个具有 SRID％u 的空间参照系。

ER_SRS_ID_ALREADY_EXISTS 在 8.0.4 中添加。
...
ER_WARN_SRS_ID_ALREADY_EXISTS 在 8.0.4 中添加。
...
ER_CANT_MODIFY_SRID_0 在 8.0.4 中添加。
...
报错信息格式：SRID 范围[％u，％u]已保留供系统使用。

在升级过程中，可以在不发出警告的情况下添加，修改或删除此范围内的 SRS。

ER_WARN_RESERVED_SRID_RANGE 在 8.0.4 中添加。
...
ER_CANT_MODIFY_SRS_USED_BY_COLUMN 在 8.0.4 中添加。
...
ER_SRS_INVALID_CHARACTER_IN_ATTRIBUTE 在 8.0.4 中添加。
...
ER_SRS_ATTRIBUTE_STRING_TOO_LONG 在 8.0.4 中添加。
...
报错信息格式：'utf8'当前是字符集 UTF8MB3 的别名，但在将来的发行版中将成为 UTF8MB4 的别名。

为了明确起见，请考虑使用 UTF8MB4。

ER_DEPRECATED_UTF8_ALIAS 已在 8.0.11 中添加。
...
报错信息格式：NATIONAL / NCHAR / NVARCHAR 表示字符集 UTF8MB3，在将来的版本中将由 UTF8MB4 代替。

为了明确起见，请考虑使用 CHAR（x）CHARACTER SET UTF8MB4。

ER_DEPRECATED_NATIONAL 已在 8.0.11 中添加。
...
报错信息格式：无效的默认归类％s：预期为 utf8mb4_0900_ai_ci 或 utf8mb4_general_ci。

ER_INVALID_DEFAULT_UTF8MB4_COLLATION 已在 8.0.11 中添加。
...
ER_UNABLE_TO_COLLECT_LOG_STATUS 已在 8.0.11 中添加。
...
ER_RESERVED_TABLESPACE_NAME 已在 8.0.11 中添加。
...
ER_UNABLE_TO_SET_OPTION 已在 8.0.11 中添加。
...
报错信息格式：对原子 DDL 语句的提交在主服务器和从服务器上均未成功。

从服务器支持原子 DDL 语句，但主服务器不支持，因此从服务器和主服务器所采取的操作可能有所不同。

在继续之前，请检查其状态是否存在差异。

ER_SLAVE_POSSIBLY_DIVERGED_AFTER_DDL 已在 8.0.11 中添加。
...
报错信息格式：功能％s 仅是为地理空间参考系统定义的，但其参数之一在 SRID％u 中，而不是地理范围。

ER_SRS_NOT_GEOGRAPHIC 已在 8.0.12 中添加。
...
报错信息格式：函数％s 遇到一个太大的多边形。

多边形必须覆盖不到地球的一半。

ER_POLYGON_TOO_LARGE 已在 8.0.12 中添加。
...
ER_SPATIAL_UNIQUE_INDEX 已在 8.0.12 中添加。
...
ER_INDEX_TYPE_NOT_SUPPORTED_FOR_SPATIAL_INDEX 已在 8.0.12 中添加。
...
ER_FK_CANNOT_DROP_PARENT 已在 8.0.12 中添加。
...
报错信息格式：功能％s 的参数包含经度％f 超出范围的几何。

它必须在（％f，％f]之内。

ER_GEOMETRY_PARAM_LONGITUDE_OUT_OF_RANGE 已在 8.0.12 中添加。
...
报错信息格式：函数％s 的参数包含纬度为％f 的几何，该几何超出范围。

它必须在[％f，％f]之内。

ER_GEOMETRY_PARAM_LATITUDE_OUT_OF_RANGE 已在 8.0.12 中添加。
...
ER_FK_CANNOT_USE_VIRTUAL_COLUMN 已在 8.0.12 中添加。
...
在参考表'％s'中缺少约束'％s'的列'％s'
ER_FK_NO_COLUMN_PARENT 已在 8.0.12 中添加。
...
ER_CANT_SET_ERROR_SUPPRESSION_LIST 已在 8.0.13 中添加。
...
报错信息格式：SRID％u 的空间参考系统定义指定了无效的地理轴'％s'和'％s'。

一个轴必须是北轴或南轴，而另一个轴必须是东轴或西轴。

ER_SRS_GEOGCS_INVALID_AXES 已在 8.0.13 中添加。
...
ER_SRS_INVALID_SEMI_MAJOR_AXIS 已在 8.0.13 中添加。


</br>

- 错误号：3738; 符号： ER_SRS_INVALID_INVERSE_FLATTENING; SQLSTATE：SR002
信息：反展平度必须大于 1.0，如果椭球是球体，则必须大于 0.0。

ER_SRS_INVALID_INVERSE_FLATTENING 已在 8.0.13 中添加。
...
ER_SRS_INVALID_ANGULAR_UNIT 已在 8.0.13 中添加。


</br>

- 错误号：3740; 符号： ER_SRS_INVALID_PRIME_MERIDIAN; SQLSTATE：SR002
信息：本初子午线必须在（-180，180]度内，以 SRS 角度单位指定。

ER_SRS_INVALID_PRIME_MERIDIAN 已在 8.0.13 中添加。
...
报错信息格式：不支持从 SRID％u 转换。

ER_TRANSFORM_SOURCE_SRS_NOT_SUPPORTED 已在 8.0.13 中添加。
...
报错信息格式：不支持转换为 SRID％u。

ER_TRANSFORM_TARGET_SRS_NOT_SUPPORTED 已在 8.0.13 中添加。
...
ER_TRANSFORM_SOURCE_SRS_MISSING_TOWGS84 已在 8.0.13 中添加。
...
ER_TRANSFORM_TARGET_SRS_MISSING_TOWGS84 已在 8.0.13 中添加。
...
ER_TEMP_TABLE_PREVENTS_SWITCH_SESSION_BINLOG_FORMAT 已在 8.0.13 中添加。
...
报错信息格式：当任何复制通道具有打开的临时表时，不允许更改@@ global.binlog_format 或@@ persist.binlog_format。

您可以等到 Slave_open_temp_tables = 0 再试一次
ER_TEMP_TABLE_PREVENTS_SWITCH_GLOBAL_BINLOG_FORMAT 已在 8.0.13 中添加。
...
报错信息格式：任何复制通道应用程序线程正在运行时，不允许更改@@ global.binlog_format 或@@ persist.binlog_format。

您可以执行 STOP REPLICA SQL_THREAD，然后重试。

ER_RUNNING_APPLIER_PREVENTS_SWITCH_GLOBAL_BINLOG_FORMAT 已在 8.0.13 中添加。
...
报错信息格式：语句违反了 GTID 一致性：@@ session.binlog_format = STATEMENT 时，不允许在事务内部或事务上下文中的过程内部创建 CREATE TEMPORARY TABLE 和 DROP TEMPORARY TABLE。

ER_CLIENT_GTID_UNSAFE_CREATE_DROP_TEMP_TABLE_IN_TRX_IN_SBR 已在 8.0.13 中添加。
...
请注意，没有主键的表可能会导致基于行的复制中的性能问题，因此在更改此设置之前，请咨询您的 DBA。

ER_TABLE_WITHOUT_PK 已在 8.0.13 中添加。
...
报错信息格式：第％ld 行的功能索引'％s'的数据被截断。

WARN_DATA_TRUNCATED_FUNCTIONAL_INDEX 在 8.0.13 中新增，在 8.0.16 之后删除。
...
ER_WARN_DATA_TRUNCATED_FUNCTIONAL_INDEX 已在 8.0.17 中添加。
...
报错信息格式：第％ld 行的功能索引'％s'的值超出范围。

ER_WARN_DATA_OUT_OF_RANGE_FUNCTIONAL_INDEX 已在 8.0.13 中添加。
...
报错信息格式：无法在返回 JSON 或 GEOMETRY 值的函数上创建函数索引。

ER_FUNCTIONAL_INDEX_ON_JSON_OR_GEOMETRY_FUNCTION 已在 8.0.13 中添加。
...
ER_FUNCTIONAL_INDEX_REF_AUTO_INCREMENT 已在 8.0.13 中添加。
...
ER_CANNOT_DROP_COLUMN_FUNCTIONAL_INDEX 已在 8.0.13 中添加。
...
ER_FUNCTIONAL_INDEX_PRIMARY_KEY 已在 8.0.13 中添加。
...
报错信息格式：无法在返回 BLOB 或 TEXT 的表达式上创建函数索引。

请考虑使用 CAST。

ER_FUNCTIONAL_INDEX_ON_LOB 已在 8.0.13 中添加。
...
ER_FUNCTIONAL_INDEX_FUNCTION_IS_NOT_ALLOWED 已在 8.0.13 中添加。
...
ER_FULLTEXT_FUNCTIONAL_INDEX 已在 8.0.13 中添加。
...
ER_SPATIAL_FUNCTIONAL_INDEX 已在 8.0.13 中添加。
...
ER_WRONG_KEY_COLUMN_FUNCTIONAL_INDEX 已在 8.0.13 中添加。
...
ER_FUNCTIONAL_INDEX_ON_FIELD 已在 8.0.13 中添加。
...
ER_GENERATED_COLUMN_NAMED_FUNCTION_IS_NOT_ALLOWED 已在 8.0.13 中添加。
...
ER_GENERATED_COLUMN_ROW_VALUE 已在 8.0.13 中添加。
...
ER_GENERATED_COLUMN_VARIABLES 已在 8.0.13 中添加。
...
ER_DEPENDENT_BY_DEFAULT_GENERATED_VALUE 已在 8.0.13 中添加。
...
ER_DEFAULT_VAL_GENERATED_NON_PRIOR 已在 8.0.13 中添加。
...
ER_DEFAULT_VAL_GENERATED_REF_AUTO_INC 已在 8.0.13 中添加。
...
ER_DEFAULT_VAL_GENERATED_FUNCTION_IS_NOT_ALLOWED 已在 8.0.13 中添加。
...
ER_DEFAULT_VAL_GENERATED_NAMED_FUNCTION_IS_NOT_ALLOWED 已在 8.0.13 中添加。
...
ER_DEFAULT_VAL_GENERATED_ROW_VALUE 已在 8.0.13 中添加。
...
ER_DEFAULT_VAL_GENERATED_VARIABLES 已在 8.0.13 中添加。
...
报错信息格式：DEFAULT 函数不能与默认值表达式一起使用。

ER_DEFAULT_AS_VAL_GENERATED 已在 8.0.13 中添加。
...
ER_UNSUPPORTED_ACTION_ON_DEFAULT_VAL_GENERATED 已在 8.0.13 中添加。
...
报错信息格式：语句违反了 GTID 一致性：ALTER TABLE ... ADD COLUMN ..表达式为 DEFAULT。

ER_GTID_UNSAFE_ALTER_ADD_COL_WITH_DEFAULT_EXPRESSION 已在 8.0.13 中添加。
...
ER_FK_CANNOT_CHANGE_ENGINE 已在 8.0.13 中添加。
...
考虑替代方案：“ SET 变量=表达式，...”或“ SELECT 表达式 INTO 变量”。

ER_WARN_DEPRECATED_USER_SET_EXPR 已在 8.0.13 中添加。
...
报错信息格式：'％s'是已弃用的字符集 UTF8MB3 的排序规则。

请考虑改用带有适当排序规则的 UTF8MB4。

ER_WARN_DEPRECATED_UTF8MB3_COLLATION 已在 8.0.13 中添加。
...
ER_WARN_DEPRECATED_NESTED_COMMENT_SYNTAX 已在 8.0.13 中添加。
...
ER_FK_INCOMPATIBLE_COLUMNS 已在 8.0.14 中添加。
...
ER_GR_HOLD_WAIT_TIMEOUT 已在 8.0.14 中添加。
...
ER_GR_HOLD_KILLED 已在 8.0.14 中添加。
...
ER_GR_HOLD_MEMBER_STATUS_ERROR 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_FAILED_TO_FETCH_KEY 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_KEY_NOT_FOUND 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_KEYRING_INVALID_KEY 已在 8.0.14 中添加。
...
报错信息格式：读取复制日志加密标头：％s 时出错。

ER_RPL_ENCRYPTION_HEADER_ERROR 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_FAILED_TO_ROTATE_LOGS 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_KEY_EXISTS_UNEXPECTED 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_FAILED_TO_GENERATE_KEY 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_FAILED_TO_STORE_KEY 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_FAILED_TO_REMOVE_KEY 已在 8.0.14 中添加。
...
报错信息格式：无法更改 binlog_encryption 值。

％s。

ER_RPL_ENCRYPTION_UNABLE_TO_CHANGE_OPTION 已在 8.0.14 中添加。
...
报错信息格式：无法恢复 Binlog 加密主密钥，请检查是否已加载密钥环插件。

ER_RPL_ENCRYPTION_MASTER_KEY_RECOVERY_FAILED 已在 8.0.14 中添加。
...
ER_SLOW_LOG_MODE_IGNORED_WHEN_NOT_LOGGING_TO_FILE 已在 8.0.14 中添加。
...
报错信息格式：选项 group_replication_consistency 不能在当前成员状态下使用。

ER_GRP_TRX_CONSISTENCY_NOT_ALLOWED 已在 8.0.14 中添加。
...
报错信息格式：在 group_replication_consistency ='BEFORE'上等待组事务提交时出错。

ER_GRP_TRX_CONSISTENCY_BEFORE 已在 8.0.14 中添加。
...
报错信息格式：等待具有 group_replication_consistency ='AFTER'的事务提交时出错。

ER_GRP_TRX_CONSISTENCY_AFTER_ON_TRX_BEGIN 已在 8.0.14 中添加。
...
报错信息格式：Group Replication 插件正在停止，因此不允许启动新事务。

ER_GRP_TRX_CONSISTENCY_BEGIN_NOT_ALLOWED 已在 8.0.14 中添加。
...
ER_FUNCTIONAL_INDEX_ROW_VALUE_IS_NOT_ALLOWED 已在 8.0.14 中添加。
...
ER_RPL_ENCRYPTION_FAILED_TO_ENCRYPT 已在 8.0.14 中添加。
...
ER_PAGE_TRACKING_NOT_STARTED 已在 8.0.16 中添加。
...
报错信息格式：未为指定的 LSN 范围启用跟踪。

ER_PAGE_TRACKING_RANGE_NOT_TRACKED 已在 8.0.16 中添加。
...
ER_PAGE_TRACKING_CANNOT_PURGE 已在 8.0.16 中添加。
...
报错信息格式：关闭“ binlog 加密”时，无法旋转二进制日志主密钥。

ER_RPL_ENCRYPTION_CANNOT_ROTATE_BINLOG_MASTER_KEY 已在 8.0.16 中添加。
...
报错信息格式：无法恢复二进制日志主密钥，new_master_key_seqno =％u，master_key_seqno =％u 和 old_master_key_seqno =％u 的组合是错误的。

ER_BINLOG_MASTER_KEY_RECOVERY_OUT_OF_COMBINATION 已在 8.0.16 中添加。
...
ER_BINLOG_MASTER_KEY_ROTATION_FAIL_TO_OPERATE_KEY 已在 8.0.16 中添加。
...
ER_BINLOG_MASTER_KEY_ROTATION_FAIL_TO_ROTATE_LOGS 已在 8.0.16 中添加。
...
ER_BINLOG_MASTER_KEY_ROTATION_FAIL_TO_REENCRYPT_LOG 已在 8.0.16 中添加。
...
ER_BINLOG_MASTER_KEY_ROTATION_FAIL_TO_CLEANUP_UNUSED_KEYS 已在 8.0.16 中添加。
...
ER_BINLOG_MASTER_KEY_ROTATION_FAIL_TO_CLEANUP_AUX_KEY 已在 8.0.16 中添加。
...
ER_NON_BOOLEAN_EXPR_FOR_CHECK_CONSTRAINT 已在 8.0.16 中添加。
...
ER_COLUMN_CHECK_CONSTRAINT_REFERENCES_OTHER_COLUMN 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_NAMED_FUNCTION_IS_NOT_ALLOWED 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_FUNCTION_IS_NOT_ALLOWED 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_VARIABLES 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_ROW_VALUE 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_REFERS_AUTO_INCREMENT_COLUMN 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_VIOLATED 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_REFERS_UNKNOWN_COLUMN 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_NOT_FOUND 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_DUP_NAME 已在 8.0.16 中添加。
...
ER_CHECK_CONSTRAINT_CLAUSE_USING_FK_REFER_ACTION_COLUMN 已在 8.0.16 中添加。
...
WARN_UNENCRYPTED_TABLE_IN_ENCRYPTED_DB 已在 8.0.16 中添加。
...
报错信息格式：请求使用％s 表空间创建％s 表。

ER_INVALID_ENCRYPTION_REQUEST 已在 8.0.16 中添加。
...
ER_CANNOT_SET_TABLE_ENCRYPTION 已在 8.0.16 中添加。
...
ER_CANNOT_SET_DATABASE_ENCRYPTION 已在 8.0.16 中添加。
...
ER_CANNOT_SET_TABLESPACE_ENCRYPTION 已在 8.0.16 中添加。
...
报错信息格式：无法加密该表空间，因为表的模式之一已将默认加密设置为 OFF，并且用户没有足够的特权。

ER_TABLESPACE_CANNOT_BE_ENCRYPTED 已在 8.0.16 中添加。
...
报错信息格式：无法解密该表空间，因为表的模式之一已将默认加密设置为 ON，并且用户没有足够的特权。

ER_TABLESPACE_CANNOT_BE_DECRYPTED 已在 8.0.16 中添加。
...
ER_TABLESPACE_TYPE_UNKNOWN 已在 8.0.16 中添加。
...
ER_TARGET_TABLESPACE_UNENCRYPTED 已在 8.0.16 中添加。
...
报错信息格式：ENCRYPTION 子句对于％s 表空间无效。

ER_CANNOT_USE_ENCRYPTION_CLAUSE 已在 8.0.16 中添加。
...
ER_INVALID_MULTIPLE_CLAUSES 已在 8.0.16 中添加。
...
报错信息格式：GRANT ... AS 仅当前受全局特权支持。

ER_UNSUPPORTED_USE_OF_GRANT_AS 已在 8.0.16 中添加。
...
报错信息格式：AS 子句中的某些授权 ID 无效或当前用户缺乏执行该语句的特权。

ER_UKNOWN_AUTH_ID_OR_ACCESS_DENIED_FOR_GRANT_AS 已在 8.0.16 中添加。
...
ER_DEPENDENT_BY_FUNCTIONAL_INDEX 已在 8.0.17 中添加。
...
不要将其添加到--early-plugin-load，keyring 迁移等中。

ER_PLUGIN_NOT_EARLY 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_START_SUBDIR_PATH 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_START_TIMEOUT 已在 8.0.17 中添加。
...
报错信息格式：服务器变量“ innodb_redo_log_archive_dirs”为 NULL 或为空。

ER_INNODB_REDO_LOG_ARCHIVE_DIRS_INVALID 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_LABEL_NOT_FOUND 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_DIR_EMPTY 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_NO_SUCH_DIR 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_DIR_CLASH 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_DIR_PERMISSIONS 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_FILE_CREATE 已在 8.0.17 中添加。
...
报错信息格式：已在'％s'上开始重做日志归档-首先调用 innodb_redo_log_archive_stop（）。

ER_INNODB_REDO_LOG_ARCHIVE_ACTIVE 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_INACTIVE 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_FAILED 已在 8.0.17 中添加。
...
ER_INNODB_REDO_LOG_ARCHIVE_SESSION 已在 8.0.17 中添加。
...
报错信息格式：正则表达式错误：函数％s 中的％s。

ER_STD_REGEX_ERROR 已在 8.0.17 中添加。
...
报错信息格式：函数％s 的参数％u 中的 JSON 类型无效；需要一个％s。

ER_INVALID_JSON_TYPE 已在 8.0.17 中添加。
...
报错信息格式：无法将字符串'％s'从％s 转换为％s。

ER_CANNOT_CONVERT_STRING 已在 8.0.17 中添加。
...
ER_DEPENDENT_BY_PARTITION_FUNC 已在 8.0.17 中添加。
...
报错信息格式：不建议使用 FLOAT / DOUBLE 列的 AUTO_INCREMENT 支持，并将在以后的版本中删除。

考虑从“％s”列中删除 AUTO_INCREMENT。

ER_WARN_DEPRECATED_FLOAT_AUTO_INCREMENT 已在 8.0.17 中添加。
...
报错信息格式：实例被锁定备份时无法停止从 SQL 线程。

尝试先运行 UNLOCK INSTANCE。

ER_RPL_CANT_STOP_SLAVE_WHILE_LOCKED_BACKUP 已在 8.0.17 中添加。
...
ER_WARN_DEPRECATED_FLOAT_DIGITS 已在 8.0.17 中添加。
...
报错信息格式：不赞成使用十进制和浮点数据类型的 UNSIGNED，并且在将来的发行版中将删除对它的支持。

ER_WARN_DEPRECATED_FLOAT_UNSIGNED 已在 8.0.17 中添加。
...
ER_WARN_DEPRECATED_INTEGER_DISPLAY_WIDTH 已在 8.0.17 中添加。
...
报错信息格式：不建议使用 ZEROFILL 属性，并将在以后的版本中将其删除。

使用 LPAD 函数将数字零填充，或将格式化的数字存储在 CHAR 列中。

ER_WARN_DEPRECATED_ZEROFILL 已在 8.0.17 中添加。
...
报错信息格式：克隆收到来自捐助方％s 的意外响应。

ER_CLONE_PROTOCOL 已在 8.0.17 中添加。
...
报错信息格式：克隆供体 MySQL 版本：％s 与收件人 MySQL 版本％s 不同。

ER_CLONE_DONOR_VERSION 已在 8.0.17 中添加。
...
报错信息格式：克隆供体 OS：％s 与收件人 OS：％s 不同。

ER_CLONE_OS 已在 8.0.17 中添加。
...
报错信息格式：克隆供体平台：％s 与收件人平台：％s 不同。

ER_CLONE_PLATFORM 已在 8.0.17 中添加。
...
报错信息格式：克隆供体排序规则：％s 在收件人中不可用。

ER_CLONE_CHARSET 已在 8.0.17 中添加。
...
报错信息格式：克隆配置％s：施主值：％s 与收件人值：％s 不同。

ER_CLONE_CONFIG 已在 8.0.17 中添加。
...
ER_CLONE_SYS_CONFIG 已在 8.0.17 中添加。
...
报错信息格式：克隆捐赠者插件％s 在收件人中未激活。

ER_CLONE_PLUGIN_MATCH 已在 8.0.17 中添加。
...
ER_CLONE_LOOPBACK 已在 8.0.17 中添加。
...
报错信息格式：克隆需要用于加密表的 SSL 连接。

ER_CLONE_ENCRYPTION 已在 8.0.17 中添加。
...
ER_CLONE_DISK_SPACE 已在 8.0.17 中添加。
...
ER_CLONE_IN_PROGRESS 已在 8.0.17 中添加。
...
ER_CLONE_DISALLOWED 已在 8.0.17 中添加。
...
ER_CANNOT_GRANT_ROLES_TO_ANONYMOUS_USER 已在 8.0.16 中添加。
...
ER_SECONDARY_ENGINE_PLUGIN 已在 8.0.14 中添加。
...
ER_SECOND_PASSWORD_CANNOT_BE_EMPTY 已在 8.0.14 中添加。
...
报错信息格式：拒绝对数据库'％s'的 AuthId'％s @％s`的访问。

ER_DB_ACCESS_DENIED 已在 8.0.16 中添加。
...
报错信息格式：无法设置必需的角色：AuthId％s @％s 具有'％s'特权。

ER_DA_AUTH_ID_WITH_SYSTEM_USER_PRIV_IN_MANDATORY_ROLES 已在 8.0.17 中添加。
...
报错信息格式：Gtid 表尚未准备好使用。

表'％s。

％s'无法打开。

ER_DA_RPL_GTID_TABLE_CANNOT_OPEN 已在 8.0.17 中添加。
...
报错信息格式：传递给函数％s 的几何位于 SRID 0 中，该 ID 未指定长度单位。

无法转换为“％s”。

ER_GEOMETRY_IN_UNKNOWN_LENGTH_UNIT 已在 8.0.14 中添加。
...
ER_DA_PLUGIN_INSTALL_ERROR 已在 8.0.17 中添加。
...
ER_NO_SESSION_TEMP 已在 8.0.13 中添加。
...
ER_DA_UNKNOWN_ERROR_NUMBER 已在 8.0.17 中添加。
...
索引'％s'的结果大小将超过％d 个字节的最大密钥长度。

ER_COLUMN_CHANGE_SIZE 已在 8.0.14 中添加。
...
ER_REGEXP_INVALID_CAPTURE_GROUP_NAME 已在 8.0.11 中添加。
...
报错信息格式：由于以下 SSL 库错误而无法设置 SSL：％s。

ER_DA_SSL_LIBRARY_ERROR 已在 8.0.17 中添加。
...
ER_SECONDARY_ENGINE 已在 8.0.13 中添加。
...
报错信息格式：不允许在已定义辅助引擎的表上使用 DDL 。

ER_SECONDARY_ENGINE_DDL 已在 8.0.13 中添加。
...
ER_INCORRECT_CURRENT_PASSWORD 已在 8.0.13 中添加。
...
报错信息格式：需要在 REPLACE 子句中指定当前密码才能进行更改。

ER_MISSING_CURRENT_PASSWORD 已在 8.0.13 中添加。
...
ER_CURRENT_PASSWORD_NOT_REQUIRED 已在 8.0.13 中添加。
...
ER_PASSWORD_CANNOT_BE_RETAINED_ON_PLUGIN_CHANGE 已在 8.0.14 中添加。
...
ER_CURRENT_PASSWORD_CANNOT_BE_RETAINED 已在 8.0.14 中添加。
...
系统变量'@@ partial_revokes'必须设置为 ON。

ER_PARTIAL_REVOKES_EXIST 已在 8.0.16 中添加。
...
报错信息格式：将 AuthId％s @％s 设置为 required_roles。

无法授予'％s'特权。

ER_CANNOT_GRANT_SYSTEM_PRIV_TO_MANDATORY_ROLE 已在 8.0.16 中添加。
...
报错信息格式：不支持将复制筛选器与 XA 事务一起使用，并且可能导致复制从属服务器处于未定义状态。

ER_XA_REPLICATION_FILTERS 已在 8.0.12 中添加。
...
报错信息格式：不支持 sql_mode = 0x％08x。

ER_UNSUPPORTED_SQL_MODE 已在 8.0.11 中添加。
...
ER_REGEXP_INVALID_FLAG 已在 8.0.12 中添加。
...
报错信息格式：数据库'％s'的'％s'特权同时作为部分撤销和 mysql.db 存在。

这可能意味着“ mysql”模式已损坏。

ER_PARTIAL_REVOKE_AND_DB_GRANT_BOTH_EXISTS 已在 8.0.16 中添加。
...
ER_UNIT_NOT_FOUND 已在 8.0.14 中添加。
...
报错信息格式：CAST 的功能索引'％s'的 JSON 值无效。

ER_INVALID_JSON_VALUE_FOR_FUNC_INDEX 已在 8.0.16 中添加。
...
报错信息格式：功能索引'％s'的 CAST 超出范围 JSON 值。

ER_JSON_VALUE_OUT_OF_RANGE_FOR_FUNC_INDEX 已在 8.0.16 中添加。
...
报错信息格式：多记录索引'％s'的每条记录的最大数值超出％u 值。

ER_EXCEEDED_MV_KEYS_NUM 已在 8.0.16 中添加。
...
报错信息格式：多值索引'％s'的每条记录的最大总值超出％u 字节。

ER_EXCEEDED_MV_KEYS_SPACE 已在 8.0.16 中添加。
...
ER_FUNCTIONAL_INDEX_DATA_IS_TOO_LONG 已在 8.0.16 中添加。
...
ER_WRONG_MVI_VALUE 已在 8.0.16 中添加。
...
ER_WARN_FUNC_INDEX_NOT_APPLICABLE 已在 8.0.16 中添加。
...
％s
ER_GRP_RPL_UDF_ERROR 已在 8.0.13 中添加。
...
报错信息格式：无法在运行组复制插件的情况下更新 GTID_PURGED。

ER_UPDATE_GTID_PURGED_WITH_GR 已在 8.0.12 中添加。
...
报错信息格式：对于具有 DST 的时区，时间上的分组是不确定的。

请考虑针对此查询切换到 UTC。

ER_GROUPING_ON_TIMESTAMP_IN_DST 已在 8.0.17 中添加。
...
ER_TABLE_NAME_CAUSES_TOO_LONG_PATH 在 8.0.4 中添加。
...
执行操作所需的角色：“％s”
ER_AUDIT_LOG_INSUFFICIENT_PRIVILEGE 已在 8.0.16 中添加。
...
ER_DA_GRP_RPL_STARTED_AUTO_REJOIN 已在 8.0.17 中添加。
...
ER_SYSVAR_CHANGE_DURING_QUERY 已在 8.0.18 中添加。
...
ER_GLOBSTAT_CHANGE_DURING_QUERY 已在 8.0.18 中添加。
...
报错信息格式：START GROUP_REPLICATION 命令无法启动其消息服务。

ER_GRP_RPL_MESSAGE_SERVICE_INIT_FAILURE 已在 8.0.18 中添加。
...
报错信息格式：通道'％s'的 SOURCE_COMPRESSION_ALGORITHMS'％s'无效。

ER_CHANGE_MASTER_WRONG_COMPRESSION_ALGORITHM_CLIENT 已在 8.0.18 中添加。
...
报错信息格式：通道'％s'的 SOURCE_ZSTD_COMPRESSION_LEVEL％u 无效。

ER_CHANGE_MASTER_WRONG_COMPRESSION_LEVEL_CLIENT 已在 8.0.18 中添加。
...
ER_WRONG_COMPRESSION_ALGORITHM_CLIENT 已在 8.0.18 中添加。
...
报错信息格式：算法'％s'的 zstd 压缩级别无效。

ER_WRONG_COMPRESSION_LEVEL_CLIENT 已在 8.0.18 中添加。
...
报错信息格式：指定的压缩算法列表'％s'超过了通道'％s'的总数 3。

ER_CHANGE_MASTER_WRONG_COMPRESSION_ALGORITHM_LIST_CLIENT 已在 8.0.18 中添加。
...
报错信息格式：复制频道'％s'的 PRIVILEGE_CHECKS_USER 设置为``@％s，但是不允许匿名用户使用 PRIVILEGE_CHECKS_USER。

ER_CLIENT_PRIVILEGE_CHECKS_USER_CANNOT_BE_ANONYMOUS 已在 8.0.18 中添加。
...
报错信息格式：复制通道'％s'的 PRIVILEGE_CHECKS_USER 设置为'％s @％s`，但这不是现有用户。

ER_CLIENT_PRIVILEGE_CHECKS_USER_DOES_NOT_EXIST 已在 8.0.18 中添加。
...
报错信息格式：在复制配置存储库中找到无效的 PRIVILEGE_CHECKS_USER，用于通道'％s'。

使用 CHANGE REPLICATION SOURCE TO PRIVILEGE_CHECKS_USER 来更正配置。

ER_CLIENT_PRIVILEGE_CHECKS_USER_CORRUPT 已在 8.0.18 中添加。
...
报错信息格式：复制通道'％s'的 PRIVILEGE_CHECKS_USER 设置为'％s @％s`，但该用户没有 REPLICATION_APPLIER 特权。

ER_CLIENT_PRIVILEGE_CHECKS_USER_NEEDS_RPL_APPLIER_PRIV 已在 8.0.18 中添加。
...
ER_WARN_DA_PRIVILEGE_NOT_REGISTERED 已在 8.0.18 中添加。
...
ER_CLIENT_KEYRING_UDF_KEY_INVALID 在 8.0.19 中添加。
...
ER_CLIENT_KEYRING_UDF_KEY_TYPE_INVALID 在 8.0.19 中添加。
...
ER_CLIENT_KEYRING_UDF_KEY_TOO_LONG 在 8.0.19 中添加。
...
ER_CLIENT_KEYRING_UDF_KEY_TYPE_TOO_LONG 在 8.0.19 中添加。
...
ER_JSON_SCHEMA_VALIDATION_ERROR_WITH_DETAILED_REPORT 在 8.0.19 中添加。
...
ER_DA_UDF_INVALID_CHARSET_SPECIFIED 在 8.0.19 中添加。
...
ER_DA_UDF_INVALID_CHARSET 在 8.0.19 中添加。
...
ER_DA_UDF_INVALID_COLLATION 在 8.0.19 中添加。
...
报错信息格式：不建议使用 VALUES 函数，并且在将来的版本中将其删除。

在这种情况下，它始终返回 NULL。

如果要访问 INSERT 语句的 VALUES 子句中的值，请考虑使用别名（INSERT INTO ... VALUES（...）AS 别名）并在 ON DUPLICATE 中引用 alias.col 而不是 VALUES（col）。

 KEY UPDATE 子句。

ER_WARN_DEPRECATED_VALUES_FUNCTION_ALWAYS_NULL 在 8.0.20 中添加。
...
ER_MISSING_JSON_VALUE 在 8.0.21 中新增。
...
ER_MULTIPLE_JSON_VALUES 在 8.0.21 中新增。
...
报错信息格式：不推荐使用 SQL_CALC_FOUND_ROWS，并将在以后的版本中将其删除。

考虑改用两个单独的查询。

ER_WARN_DEPRECATED_SQL_CALC_FOUND_ROWS 已在 8.0.16 中添加。
...
报错信息格式：不建议使用 FOUND_ROWS（），并将在以后的版本中将其删除。

考虑改用 COUNT（*）。

ER_WARN_DEPRECATED_FOUND_ROWS 已在 8.0.16 中添加。
...
报错信息格式：主机名不能超过％d 个字符。

ER_HOSTNAME_TOO_LONG 在 8.0.21 中新增。
...
报错信息格式：不赞成使用 PARTITION BY KEY（）子句中具有前缀键部分'％s（％u）'的列'％s。

％s。

％s'，并将在以后的版本中将其删除。

ER_WARN_CLIENT_DEPRECATED_PARTITION_PREFIX_KEY 在 8.0.21 中新增。
...
报错信息格式：由于为恢复通道提供的用户名为空，因此 START GROUP_REPLICATION 命令失败。

ER_GROUP_REPLICATION_USER_EMPTY_MSG 在 8.0.21 中新增。
...
报错信息格式：START GROUP_REPLICATION 命令失败，因为未为恢复通道提供 USER 选项。

ER_GROUP_REPLICATION_USER_MANDATORY_MSG 在 8.0.21 中新增。
...
报错信息格式：由于为恢复通道提供的密码超过了 32 个字符的最大长度，因此 START GROUP_REPLICATION 命令失败。

ER_GROUP_REPLICATION_PASSWORD_LENGTH 在 8.0.21 中新增。
...
报错信息格式：语句要求将子查询转换为非 SET 操作（例如 IN2EXISTS 或 subquery-to-lateral-derived-table）。

使用优化程序开关'subquery_to_derived'不允许这样做
ER_SUBQUERY_TRANSFORM_REJECTED 在 8.0.21 中新增。
...
ER_DA_GRP_RPL_RECOVERY_ENDPOINT_FORMAT 在 8.0.21 中新增。
...
ER_DA_GRP_RPL_RECOVERY_ENDPOINT_INVALID 在 8.0.21 中新增。
...
％s
ER_WRONG_VALUE_FOR_VAR_PLUS_ACTIONABLE_PART 在 8.0.21 中新增。
...
报错信息格式：在带有 START TRANSACTION 语句的 CREATE TABLE 之后，仅允许 BINLOG INSERT，COMMIT 和 ROLLBACK 语句。

ER_STATEMENT_NOT_ALLOWED_AFTER_START_TRANSACTION 在 8.0.21 中新增。
...
报错信息格式：使用 CREATE TABLE 作为 SELECT 以及使用 START TRANSACTION 语句的 CREATE TABLE 不允许外键创建。

ER_FOREIGN_KEY_WITH_ATOMIC_CREATE_SELECT 在 8.0.21 中新增。
...
报错信息格式：不能使用 START TRANSACTION 子句％s。

ER_NOT_ALLOWED_WITH_START_TRANSACTION 在 8.0.21 中新增。
...
报错信息格式：无效的 json 属性，错误：位置％u 处的“％s”：“％s”。

ER_INVALID_JSON_ATTRIBUTE 在 8.0.21 中新增。
...
报错信息格式：存储引擎'％s'不支持 ENGINE_ATTRIBUTE。

ER_ENGINE_ATTRIBUTE_NOT_SUPPORTED 在 8.0.21 中新增。
...
报错信息格式：用户属性必须是有效的 JSON 对象。

ER_INVALID_USER_ATTRIBUTE_JSON 在 8.0.21 中新增。
...
报错信息格式：由于 InnoDB 重做日志已禁用，因此无法执行操作。

使用 ALTER INSTANCE 启用重做日志后，请重试。

ER_INNODB_REDO_DISABLED 在 8.0.21 中新增。
...
报错信息格式：无法执行操作，因为 InnoDB 正在归档重做日志。

请通过调用 `innodb_redo_log_archive_stop()` 停止重做存档后重试。

ER_INNODB_REDO_ARCHIVING_ENABLED 在 8.0.21 中新增。
...
ER_MDL_OUT_OF_RESOURCES 在 8.0.21 中新增。
