...
- R 表示修订版本：MYSQL_VERSION_REVISION，例如 5.7.36-39 Revision a2ce7ad3400、8.0.25-16 Revision 202302041847、8.0.32-27 Revision aa66a385910 等
- GreatSQL 的大版本号与 Percona Server for MySQL/Oracle MySQL 版本号对应。

以 GreatSQL 8.0.32-27 版本为例，登录到 GreatSQL 数据库后，执行 `\s` 或 `STATUS` 命令就可以得到以下信息：
...
在 GreatSQL 中执行 SQL 命令 `SHOW DATABASES` 查看数据库列表：
