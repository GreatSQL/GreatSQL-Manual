...
MGR 是从 MySQL 5.7.17 开始引入的。但 MySQL 5.7 已于 2020 年 10 月起不再做大的功能更新，仅提供安全补丁。因此，如果线上还有基于 MySQL 5.7 版本的 MGR 环境，建议尽快升级到 GreatSQL 8.0 版本。
...
事务从 Server 层通过钩子（hook）进入 MGR API 接口层，再分发到各组件层，在组件层完成事务 Capture/Apply/Recover，通过复制协议层（Replication Protocol Logics）传输事务，最后经由 GCS 协调事务在各节点的最终一致性。
